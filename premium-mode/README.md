...
- **초도 발행 프로모션 예고**: 위 무르기 무료 전환은 "영구 무료"가 목표가 아니라, 초도 발행 시점의 **클레임 기반 프로모션 + 그랜드파더링**으로 다시 좁혀질 예정이다(아직 미구현). 구체 설계는 `launch-plan/README.md` 3장 참고 — 이후 정책이 바뀌어도 초기에 클레임한 유저는 계속 무료로 유지하는 것이 핵심이라, 단순 "무르기 유료 전환"과는 다르다.
