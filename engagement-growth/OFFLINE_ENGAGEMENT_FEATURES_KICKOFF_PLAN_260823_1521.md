...
| 일차 | 지급 보상 |
| --- | --- |
| 1일차 | ① **'무제한 무르기' 기능 영구 활성화**(기존 `FeatureId.Undo` claim 재사용 — 4.4절) ② **첫 번째 AI 봇 캐릭터** 획득 |
| 2일차 | **'단발성 형세 보기' 10개** (4.5절) |
| 3일차 | **'단발성 추천 수' 10개** (4.5절) |
| 4일차 | **'단발성 프리미엄 모드 활성화(광고 스킵권)' 10개** (4.5절) |
| 5일차 | **두 번째 AI 봇 캐릭터** 획득 |
| 6·7일차, 14/21/28일차 | **미확정** — 남은 캐릭터 3종(중수·고수·초고수)을 여기서 줄지, 광고 시청(7장)으로 돌릴지 함께 정해야 한다. 지금 코드에는 임시로 광고 획득으로 달려 있다(7장 #8 각주). |

- 보상 지급 방식은 **자동 지급이 아니라 Claim 방식**이다 — 5.1절 참고.
- 캐릭터의 "첫 번째/두 번째"는 `FastBeginner` 티어 오름차순(초보 → 하수)을 뜻한다. 즉 첫 실행에
  받는 캐릭터가 가장 약한 상대이고, 강한 상대일수록 나중에 열린다.
- ⚠️ 이 표에 따라 **AI 난이도 5단계가 전부 잠긴다**(획득해야 고를 수 있다). 지금까지 조건 없이
  고를 수 있던 선택지가 좁아지는 것은 **의도된 방향으로 확정됐다**(2026-08-24).
