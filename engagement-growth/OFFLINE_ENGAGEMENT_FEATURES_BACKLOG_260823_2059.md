...
> 게이트에 막혀 진행 중에 멈춰 있다. 예정사항에 남은 것은 아래 #24 하나다.

24. 1회권 재고를 대국 화면 밖으로 빼고 버튼 자체에 남은 수를 녹이기 (AI 모델: Opus, 노력정도: 중간)
    ...
    - 착수 전 확인(사용자): **"마이 페이지"가 지금 앱에 없다.** `ScreenDestination`은 Onboarding/Home/Settings/Study/GameHistory/GameSetup/InGame뿐이다. 셋 중 하나를 골라야 한다 — ⓐ 설정 화면 안의 한 절, ⓑ 홈에 카드 하나 추가(대국 기록 옆), ⓒ 새 목적지 신설. ⓐ가 가장 싸고 ⓒ가 가장 확장성 있다(출석 현황·캐릭터 컬렉션도 나중에 여기 붙을 자리다).
    - 관련: 문구는 4개 언어가 필요하다. `∞`는 번역 대상이 아니라 기호이므로 언어별로 같다.
