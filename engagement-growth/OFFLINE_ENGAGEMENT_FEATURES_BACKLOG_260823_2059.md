...
- 아직 **미확정**인 것(2026-08-24 기준 2건으로 줄었다): ① **6·7일차와 14/21/28일차 보상 내용**, ② **남은 캐릭터 3종(중수·고수·초고수)의 획득 경로** — 코드에는 임시로 광고 획득(`BotUnlockSource.AdWatch`)이 달려 있고, 이 둘은 사실상 같은 질문이다(출석 보상으로 줄지 광고로 줄지). #11에서 확정한다. / ~~'광고 스킵권' 1회의 범위~~는 #12에서, ~~Claim 안 한 보상의 만료 여부~~는 #14에서 확정됐다. **미확정 항목은 스레드가 대신 정하지 말고 그 시점에 사용자에게 확인하세요.**
...
> 항목을 가져간다.** 끼어들었던 항목이 모두 끝나 **남은 순서는 10 → 11**이다.
