...
> 게이트에 막혀 진행 중에 멈춰 있다. #24·#25가 끝나고 #27·#28에 착수했으며, 예정사항은 아래 #26·#29다.
...
29. 큰 글꼴 배율에서 잘리는 나머지 자리들 (AI 모델: Sonnet, 노력정도: 낮음)
    ...
    - ⓑ **홈의 `MenuCard` 부제**. 카드가 `height(120.dp)` 고정이라 배율 2.0배에서 부제가 통째로 잘린다(실기 확인). 카드 4장 모두 해당된다.
    - **검토할 수단**: 라벨 자체를 줄이거나(`새 대국 시작` → `새 대국`), `ActionButtonContentPadding`의 좌우 8dp를 낮춰 가용폭을 되찾거나(검증 에이전트 제안 — 다만 버튼 5개의 여백이 모두 얇아진다), `MenuCard`를 `height` → `heightIn`으로 바꾸는 것.
    - ⚠️ **가용폭 추정치를 다시 재라.** #27 조사는 144dp/88dp로 계산했는데, 검증에서 M3 `Button` 내부 패딩을 감안하면 **152dp/96dp**라는 지적이 나왔다. 임계 배율이 그만큼 달라진다.
