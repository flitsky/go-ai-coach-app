...
- **핵심 UI 요소**:
  ...
  2. **대국 설정 패널 (`ScoringAndBoardSettingsPanel`)** — 아래 순서로 배치:
     - **계가 방식 (`Scoring Rule`)**: `면적 계가 (영역+돌 계가)` (Area/중국식) / `집 계가 (영역 계가)` (Territory/한국·일본식)
...
| **집 계가** | 집 계가 (영역 계가)[^1] | Territory Scoring | 地合計算 | 数目计分 |
...
[^1]: 실제 앱 코드(`UiStrings.kt`)의 문구를 그대로 옮긴 것입니다. "집 계가"(Territory 규칙)인데 괄호 안에 "영역 계가"(Area 규칙의 명칭)가 붙어 있어 자기모순처럼 보입니다 — 문서가 아닌 코드 쪽의 표기 이슈로 추정되며, "앱 텍스트 우선" 원칙에 따라 여기서는 수정하지 않고 실제 문구를 그대로 기록합니다. 필요 시 `UiStrings.kt`의 이 라벨 자체를 별도로 검토하세요.
