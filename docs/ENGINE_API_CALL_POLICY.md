...
`deadStones()`와 `scoreFinal()`은 엔진 코어 API로는 원시 기능을 그대로 노출하되, 앱 UX에서는 그대로 무제한 동기 실행하지 않는다. 2026-06-14 실기기 계측에서 `Time cap OFF` 상태의 같은 종국 국면이 `deadStonesMs=17873`, `finalScoreMs=28845`, `totalWithSyncMs=46964`를 기록했다.

운영 정책은 다음과 같다.

- 대국 중 `Search Time = OFF`라도 기본 종국 판정은 5초 SLA를 가진다.
- 화면에는 부심 판정, 즉 빠른 로컬/짧은 제한 엔진 조합 결과를 먼저 보여준다. 5초 안에 엔진 종국 결과가 충분히 오지 않으면 로컬/기존 분석 기반의 불확정 결과를 표시한다.
