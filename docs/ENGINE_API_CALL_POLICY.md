...
현재 startup benchmark는 실제 대국 속도를 그대로 예측하는 기능이 아니다. 기기 진단을 위해 B16/B32/B64를 모두 `timeCapMs=5000`으로 측정한다. 반면 `빠른 초급` 실제 대국은 `16 visits / 250ms` fast path를 사용한다.

따라서 benchmark 평균이 2-5초로 느려도, `빠른 초급 3단계` 대국이 250-400ms 수준으로 쾌적하게 느껴질 수 있다. benchmark 결과는 “장시간 진단 호출에서 이 기기가 visits를 얼마나 채우는가”를 보는 값이고, 실제 대국 체감은 목적별 `TurnAnalysis` 예산을 기준으로 판단한다.
