...
| 4 | `deadStones()` | 낮음~높음 | 기본 종국 pass/pass | 사석 정리. 기본 빠른 종국은 assistant judge 5초 cap을 주입하고 진단용 `scoreFinal()`은 호출하지 않음 |
| 5 | `scoreFinal()` | 높음~매우 높음 | 기본 경로 비활성 | KataGo 원문 최종 계가 진단. 실기기에서 수십 초까지 늘 수 있어 향후 이의신청/정밀 계가 경로에서만 사용 |
...
`deadStones()`와 `scoreFinal()`은 엔진 코어 API로는 원시 기능을 그대로 노출하되, 앱 UX에서는 그대로 무제한 동기 실행하지 않는다. 2026-06-14 실기기 계측에서 `Time cap OFF` 상태의 같은 종국 국면이 `deadStonesMs=17873`, `finalScoreMs=28845`, `totalWithSyncMs=46964`를 기록했다. 또한 5초 `maxTime`을 두 raw 명령에 각각 적용하면 체감상 약 10초가 될 수 있으므로, 기본 빠른 종국에서는 진단용 `scoreFinal()`을 생략한다.

운영 정책은 다음과 같다.

- 대국 중 `Search Time = OFF`라도 기본 종국 판정은 5초 assistant judge cap을 가진다.
- 현재 구현은 pass/pass 종국 직전에 `EngineProfile.analysisLimit.timeMillis=5000`으로 `configure()`를 다시 호출해 KataGo GTP process-global `maxTime`을 제한한다.
- 기본 화면 결과는 `deadStones()` + 로컬 사석 fallback + 로컬 계가 + 기존 NN/Top Moves 추정 조합으로 만든다. 진단용 `scoreFinal()`은 기본 경로에서 호출하지 않고 debug log에 `diagnosticKataGoFinalScoreSkipped=skipped-by-assistant-judge-sla`를 남긴다.
