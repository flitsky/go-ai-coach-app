...
| 파일 | 라인 수 | 역할 |
| --- | ---: | --- |
| `app-android/.../ui/GoCoachApp.kt` | 1,957 | 화면, 상태, 엔진 orchestration, 종국, 저장/복원, debug report |
...
- `match`와 `persistence`가 생기면서 Android UI 밖으로 일부 책임이 이미 빠져나가기 시작했다.
...
- Debug report 문자열 생성이 UI 파일 하단에 붙어 있어 테스트와 재사용성이 낮다.
...
다음 작업으로는 Phase 1을 추천한다.

첫 커밋:

- `DebugReportBuilder` 분리
- 관련 formatter 이동
- 테스트 추가
- 기능 변화 없음
...
- `AnalysisCacheStore`, `AnalysisRequestFactory` 분리
- Top Moves helper와 cache key 이동

이 순서가 좋은 이유는 UI 화면 로직을 건드리는 폭이 작고, 실패해도 원인 추적이 쉽기 때문이다. 이후에 `GameSessionController`나 ViewModel을 도입하면 상태 전이를 옮길 공간이 이미 생긴다.
