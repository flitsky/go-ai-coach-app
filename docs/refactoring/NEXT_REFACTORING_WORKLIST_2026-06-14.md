...
- 2026-06-14: 종국 빠른 판정 정책을 실제 코드에 연결했다. pass/pass 종국 직전에 `AssistantJudgeEndgameTimeCapMillis=5000`을 주입해 `configure()`하고, 기본 경로에서는 체감 10초 누적 원인이 될 수 있는 진단용 `scoreFinal()` 호출을 생략한다. debug log에는 assistant judge cap과 `scoreFinal` skip 사유를 남긴다.
