...
- `GameSessionUiStateHolderApplication.kt`는 Compose state를 직접 소유하지 않고 current/apply callback만 받는다. 따라서 KMP 후보라기보다 UI state mutation을 application reducer 경계로 모으는 중간 어댑터로 보는 것이 정확하다.
