...
- `ScoreDisplayFormatterApplication.kt`는 final/endgame display text 조립만 담당한다. 현재 Android/UI/persistence/runtime 구현 의존이 없어 KMP 이동 후보로 가볍다. 다만 문구 자체를 shared/common에 둘지, locale/resource 기반 Android formatter로 둘지는 별도 제품 결정이 필요하다.
