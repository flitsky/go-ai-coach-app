...
2026-06-12 이후의 세부 리팩토링 절차는 `docs/refactoring/DOMAIN_SEPARATION_REFACTORING_PLAN.md`를 기준으로 진행한다. 핵심은 엔진 원시 기능을 `Engine Core API`로 빠짐없이 노출하고, 앱/서버 공통 미들웨어가 이를 조합하며, Game UX는 로컬 엔진인지 원격 서버인지 모르고 미들웨어 API만 호출하게 하는 것이다.
...
- 자동 AI 턴 display state applier 분리
...
- `EngineAdapter`는 기존 호환 이름으로 유지하되, 의미상 엔진 원시 기능 1:1 계약은 `EngineCoreApi`로 명시한다.
- `EngineSessionClient`는 미들웨어 API이며, UX가 호출하는 기본 엔진 경계다.
- 흑/백 진영, AI 캐릭터, 게임 설정, 심판/대국 진행 도메인은 미들웨어 계층에서 점진적으로 분리한다.
- 원격 서버 엔진은 `EngineCoreApi` 또는 `EngineSessionClient` 구현체로 들어올 수 있어야 하며, UX 코드는 이 차이를 몰라야 한다.
