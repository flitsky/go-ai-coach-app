...
갱신: 2026-07-30 — `ARCHITECTURE.md`를 앱 비종속 원칙 문서로 전면 재작성하고, go-ai-coach 구체 매핑·로드맵을 담은 `GO_AI_COACH_ARCHITECTURE_ROADMAP.md`를 신설했다(핵심 문서 9개 → 10개). `premium-mode/`, `auth-onboarding/`, `ux-improvement/`, `docs/history/baas_solutions_comparison.md`, `docs/history/baduk_app_architecture_recommendation.md`를 이 인덱스에 편입했다. 상세는 "문서 이력" 절 참고.  
...
갱신: 2026-08-17 (3) — **"삭제 대신 보관" 문서 보존 원칙을 뒤집었다.** `docs/archive/`(5개 하위 폴더, 55개 파일, 1.2MB)와 `docs/refactoring/`(리팩토링 축이 260817에 종료 확인됨, 8개 파일)를 저장소에서 완전히 제거하고 git 히스토리로만 남겼다 — 사유와 복원 방법은 아래 "문서 보존 정책" 절 참고. 유일한 예외로 실측 데이터가 계속 인용되던 `ENGINE_BEGINNER_VISITS_BENCHMARK.md`는 `docs/engine/`로 이동 보존했다. 삭제된 경로를 가리키던 모든 교차 참조(`ENGINE_API_CALL_POLICY.md`, `GO_AI_COACH_ARCHITECTURE_ROADMAP.md`, `docs/history/THREAD_HISTORY.md`, `launch-plan/README.md`, `premium-mode/README.md`, `auth-onboarding/README.md`)를 함께 정리했다. 아래 "리팩토링 전략/진행 로그"·"아카이브" 두 섹션은 이 갱신으로 제거됐다.
...
갱신: 2026-08-24 — AI 스레드 협업 방법론을 코드베이스에 안 묶인 이식용 표준 가이드로 정리해 `docs/history/knowhow-docs/_SPEC_PLAN.md`+`_SPEC_BACKLOG.md`를 신설했다(위 "작업 방법론 문서" 표에 편입). 외부 초안을 검증 없이 그대로 채택하지 않고 예시 코드가 실제 코드베이스와 일치하는지 매번 grep으로 재확인했다 — 두 차례에 걸쳐 실재하지 않는 클래스(`ConsumableInventoryCoordinator`)와 실제와 다른 인터페이스 시그니처가 섞여 있던 것을 발견해 전부 검증된 실제 코드 또는 완전한 플레이스홀더로 교체했다.
...
| `docs/measurements/engine-match/`, `docs/measurements/engine-benchmark/` | 엔진 성능·매치 raw/summary 로그 | 데이터, 구조 고정. **깊이 규칙 적용 대상 밖** | "데이터 로그" |
| `docs/history/knowhow-docs/` | AI 스레드 협업 방법론(코드베이스 비종속 이식용 가이드) | 표준 문서, 새 프로젝트로 복사해 이식 | "작업 방법론 문서" |
| `docs/history/market-listing-history/` | 마켓(Play 등) 등록/심사에서 반복되는 절차·허들과 대응 기록 | 앱을 낼 때마다 append. **이 앱 전용이 아니라 다음 앱에도 재사용하는 지식자산** | "마켓 등록 히스토리" |
...
| `docs/roadmap/` | 앱 **고도화 계획과 진행 이력** + 진행 중인 리팩토링 로드맵(옛 `docs/refactoring/` 흡수)(아이디어 → 착수 스펙 → 일감 백로그 → 완결) | 파일명이 `시작일-완결일_이름.md`라 **이름순 정렬이 곧 시간순**이다. 폴더 `README.md`가 규칙과 현재 목록을 담는다 | "앱 고도화 트랙" |
...
| `HANDOVER.md` | **개발 인수인계 진입점.** 새 개발자(또는 새 AI 에이전트 계정)가 처음 여는 문서 — 지금 이어받을 트랙의 파일 하나와 고정 프롬프트, 일감 사이클, 트랙 종료 후 다음 트랙을 여는 절차, 빌드 환경 함정. 방법론 규칙 자체는 `history/knowhow-docs/`로 위임한다 |
...
| `docs/history/baas_solutions_comparison.md` | Firebase/Supabase/PocketBase/Appwrite/Convex BaaS 비교 조사 — Firebase 채택 근거 원본. 결론은 `auth-onboarding/README.md`에 반영됨 |
| `docs/history/baduk_app_architecture_recommendation.md` | 백엔드+AdMob 전략 추천 조사 — 결론은 `premium-mode/README.md`/`auth-onboarding/README.md`에 반영됨 |
...
- **원시 측정 데이터**(`docs/measurements/`) — 깊이 상한을 적용하지 않는다. ⚠️ 근거: 이 데이터는
...
예외 두 곳과, 폴더 경로를 지목하는 참조(`docs/measurements/...`)뿐이다.

### 현재 상태 (2026-08-31 — 세 상한 모두 충족)

| | 현재 | 상한 |
| --- | --- | --- |
| 최상위 문서 | **5개** ✓ (+ 이 색인, 적용 대상 밖) | 5 |
| 하위 폴더 | **5개** ✓ — `spec/`·`engine/`·`measurements/`·`roadmap/`·`history/` | 5 |
| 최대 깊이(문서) | **3** ✓ (`engine/error-cases/`, `history/knowhow-docs/`, `history/market-listing-history/`) | 3 |
| 최대 깊이(`measurements/`) | 4 — 적용 대상 밖 | — |

⚠️ **깊이가 2에서 3으로 올라갔다.** 상한 안이지만 **여유가 없어졌다** — 이제 어떤 폴더든 한 겹 더
씌우면 규칙을 넘는다. 다음에 하위 폴더를 만들고 싶어지면 그 자리에 **이미 3뎁스가 있는지** 먼저 볼 것.

⚠️ **`spec/`은 원래 계획에서 `architecture/`였다**(2026-08-31 사용자 확정으로 개명). 그 폴더에는
계층 매핑만 들어가는 것이 아니라 **출석 보상 정책·사용자 옵션처럼 아키텍처가 아닌 제품 명세**가
함께 들어가서, `architecture/`라는 이름이 폴더 내용에 대해 거짓이 된다.

### ⚠️ 상한을 채운 뒤의 규칙

세 상한이 모두 **정확히 찬 상태**다. 그래서 앞으로는 **"들어가려면 하나가 나와야 한다"가 선택이
아니라 유일한 방법**이다 — 최상위에 문서를 더하려면 하나를 `spec/`이나 `engine/`으로 내리고,
새 중분류가 필요하면 기존 다섯 중 하나를 접어야 한다. 개수를 슬쩍 늘리는 쪽으로 가면 이 저장소가
2026-08-29에 겪은 3중 불일치로 돌아간다.
...
## 리팩토링 전략/진행 로그 (`docs/roadmap/` 안)

⚠️ **`docs/refactoring/` 폴더는 없어졌다**(백로그 #58, 중분류 8 → 5). 리팩토링 로드맵은 성격이
고도화 계획과 같아 `docs/roadmap/`으로 흡수했다 — 폴더를 되살리지 말 것.
여전히 **끝나면 삭제한다**는 보존 정책은 그대로 적용된다.

| 위치 | 용도 |
| --- | --- |
| `roadmap/LAYERED_ARCHITECTURE_REFACTORING_PLAN_260803_1500.md` | 7계층 원칙을 실제 코드에 단계적으로 반영하는 로드맵. Stage D(로컬/원격 `EngineCoreApi` 계약 대등화)·Stage E(`RemoteEngineSessionClient`, 원격 후보 선택)는 260804에 완료. Stage E-3(개발용 HTTP 참조 서버 + 디버그 토글 배선, 260818)까지 실제 대국 e2e 검증 완료. Stage F(물리적 분산·DePIN)는 전용 킥오프 문서로 분리(아래 행) |
| `roadmap/REMOTE_ENGINE_MQ_TRANSPORT_KICKOFF_PLAN_260818_0825.md` | Stage F 전용 킥오프 — HTTP→MQ/Firestore 전환, 폰↔폰 지원, 세션 토픽 기반 정합성 체크·보상 점수 감사로그 설계. **결정 문서, 착수 전** — "맥북 파이썬으로 먼저 검증 후 앱 이식" 결정과 근거만 기록, 구현은 별도 승인 대기 |
...
| 위치 | 용도 |
| --- | --- |
| `history/market-listing-history/README.md` | 구글 플레이 등 마켓의 등록/심사 절차에서 반복적으로 마주치는 허들과 대응 방법, 관련 캡처 기록 |
...
| `history/baduk_app_architecture_recommendation.md` | 2026-07-30 이전 초기 조사 — 바둑 앱 아키텍처 후보 비교. 결론은 `ARCHITECTURE.md`/`PRD.md`에 흡수됐다. **요약/딥다이브 체계 밖의 일회성 리서치**라 최상위가 아닌 여기에 둔다(백로그 #58) |
| `history/baas_solutions_comparison.md` | 2026-07-30 이전 초기 조사 — BaaS 후보 비교. 결론은 `auth-onboarding/README.md`가 이어받았다. 위와 같은 성격이다 |
...
| 위치 | 용도 |
| --- | --- |
| `measurements/engine-match/` | 맥북 KataGo 레벨 매트릭스 raw/summary 로그 |
| `measurements/engine-benchmark/` | 실기기/에뮬레이터 엔진 성능 및 자동대국 진단 로그 |
