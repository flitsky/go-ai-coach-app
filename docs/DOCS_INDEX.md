...
| 위치 | 용도 |
| --- | --- |
| `docs/AI_COLLABORATION_BACKLOG_METHOD.md` | **AI 스레드와 협업하는 단일 문서 백로그 방법론**(2026-08-24 작성, 같은 날 외부 v2.0 초안 비판적 검토 + 포지셔닝 재정리까지 2차 개정). 핵심 가치 두 축을 명시적으로 다룬다 — ① **팀 상위 이슈 트래커(Jira 등) 아래 개인이 할당받은 일감을 이 문서로 추적**하는 계층 구조, ② **한 사람이 여러 프로젝트를 오갈 때 맥락을 잃지 않는** 다중 프로젝트 강점. `engagement-growth/` 백로그에서 실제로 굴려본 운영 규칙 10가지, 조직 트래커·보고 연계 방법, 겪은 마찰과 대응, 이식용 빈 템플릿, 이슈 트래커와의 비교, 저장소 소급 적용은 하지 않고 낮은 위험 후보만 참고용으로 남긴 절까지 포함. **새 트랙을 백로그 방식으로 시작하려는 사람은 이 문서의 8장 템플릿을 복사해 쓰면 된다.** 제품 문서가 아니라 일하는 방식 문서라 위 두 분류(핵심 문서/마스터플랜) 어디에도 속하지 않아 별도 절로 둔다 |
