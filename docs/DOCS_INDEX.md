...
갱신: 2026-08-23 — 일일 접속량(DAU) 증대 아이디어를 모으는 브레인스토밍 문서 `docs/roadmap/260823-_DAU_GROWTH_IDEAS.md`를 신설해 "저장소 루트의 마스터플랜 폴더" 표에 편입했다. chess.com의 재방문 유도 기능(Daily Puzzle, Puzzle Rush, 봇 캐릭터, Game Review 등)을 참고 사례로 정리하고, 우리 앱의 현재 조건(로그인 꺼짐, 서버 없음, 멀티플레이 없음, 푸시 알림 인프라 없음 — 2026-08-23 코드베이스로 재확인)에 맞는 적용 아이디어를 상태 트래킹 표(제안/논의중/채택/보류/기각)로 관리한다. 아직 결정된 것은 없는 초안 단계 — 계속 논의하며 갱신 예정.
갱신: 2026-08-23 (2) — 위 아이디어 중 출석 보상·AI 캐릭터화·업적 화면·대국 히스토리 4개를 "로그인 없이 로컬 전용(Phase 1)" 방향으로 구체화한 착수 계획서 `docs/roadmap/260823-260830_OFFLINE_ENGAGEMENT_FEATURES_KICKOFF_PLAN.md`를 신설했다. 리팩토링이 아니라 신규 기능 착수 계획서라 `docs/refactoring/`가 아니라 주제 폴더(`docs/roadmap/`) 안에 두는 것으로 배치 기준을 정했다 — 이 문서부터 "착수 계획서" 유형이 반드시 `docs/refactoring/`에만 있는 게 아니라는 점을 명확히 함. 로그인 연동(Phase 2)은 범위 밖으로 명시적으로 분리하되, 기존 Port 인터페이스(`shared`)+SharedPreferences 어댑터(`app-android/persistence`) 패턴을 그대로 따르게 해 나중에 Firestore 어댑터만 추가하면 확장되도록 설계했다. **결정 문서, 착수 전** — 새 스레드로 바로 넘겨 개발을 시작할 수 있는 형태.
갱신: 2026-08-23 (3) — 위 킥오프 플랜을 새 스레드마다 하나씩 순차 착수 가능한 11개 일감(완료/진행중/예정 상태 + AI 모델·노력정도 표기)으로 쪼갠 백로그 `docs/roadmap/260823-260830_OFFLINE_ENGAGEMENT_FEATURES_BACKLOG.md`를 신설했다. 사용자 피드백으로 "봇 캐릭터 선택이 곧 AI 레벨(초보~초고수) 선정" 방향이 확정돼 킥오프 플랜 7.1절에 반영, 백로그 #10(진입점 UX 개편)에 노력정도 최대로 표기했다.
...
갱신: 2026-08-24 (2) — "왜 마스터플랜 폴더가 `docs/` 밖에 있는가"를 물어온 사용자 질문에 답하며, 그 답이 문서 중간에 묻혀 있어 저장소 소유자도 찾기 어려웠던 것을 확인했다. "하위 폴더 한눈에 보기"를 `docs/`·저장소 루트를 통틀은 "문서/폴더 전체 지도"로 확장하고, 배치 근거(참조 안정성 — 각 마스터플랜 백로그의 고정 프롬프트가 자기 경로를 하드코딩하고 있어 이동하면 조용히 깨짐, 2026-08-18 사건이 실례)를 명시적 절로 승격했다. `docs/history/`에 완전히 종료된 마스터플랜 트랙이 편입될 때 쓸 ISO 날짜 접두어 규약(`YYYY-MM-DD_<폴더명>`)도 "프로젝트 히스토리" 절에 신설 — 활성 폴더에는 적용하지 않는다. `engagement-growth/`는 백로그 미완료(#16~#18) 상태라 이동하지 않고 계속 루트에 남긴다(사용자 결정). **[2026-08-30 후일담: 백로그가 완결되면서 이 조건이 사라져 `docs/roadmap/`으로 옮겼다.]**
...
| `feature-access-principles/`, `launch-plan/`, `premium-mode/`, `auth-onboarding/`, `ux-improvement/`, `design-handoff/` (전부 저장소 **루트**) | 기능별로 계속 갱신되는 마스터플랜 | 활성 트래커. 완료돼도 이동하지 않고 이 인덱스로만 관리(예외: 완전 종료 시 `docs/history/`로) | "저장소 루트의 마스터플랜 폴더" |
| `docs/roadmap/` | 앱 **고도화 계획과 진행 이력**(아이디어 → 착수 스펙 → 일감 백로그 → 완결) | 파일명이 `시작일-완결일_이름.md`라 **이름순 정렬이 곧 시간순**이다. 폴더 `README.md`가 규칙과 현재 목록을 담는다 | "앱 고도화 트랙" |
...
> 백로그가 완결되면서 그 전제가 사라져 `docs/roadmap/`으로 내렸다(루트 폴더 수를 줄이려는
...
| `docs/roadmap/260823-_DAU_GROWTH_IDEAS.md` | 일일 접속량(DAU) 증대 아이디어 브레인스토밍 — chess.com 참고 사례와 우리 앱 조건에 맞춘 적용 아이디어를 상태 트래킹 표로 관리. 2026-08-23 초안 단계, 계속 논의하며 갱신 |
| `docs/roadmap/260823-260830_OFFLINE_ENGAGEMENT_FEATURES_KICKOFF_PLAN.md` | 위 아이디어 중 출석 보상·업적 화면·대국 히스토리·봇 컬렉션을 "로그인 없이 로컬 전용(Phase 1)"으로 구체화한 개발 착수 스펙. 새 스레드에 그대로 넘겨 바로 개발 가능한 형태 |
| `docs/roadmap/260823-260830_OFFLINE_ENGAGEMENT_FEATURES_BACKLOG.md` | 위 스펙을 새 스레드마다 하나씩 순차 착수 가능한 일감으로 쪼갠 진행 관리 백로그. **2026-08-30 완결 — 더 이상 갱신하지 않는다**(#40 스크린샷 재캡처 + 비공개 테스트 AAB v0.8.4가 마지막). #1~#40의 완료 이력과 "신규 스레드 착수 프로토콜"은 참조용으로 그대로 남아 있다. **새 일감은 아래 문서가 받는다** |
| `docs/roadmap/260830-_POST_LAUNCH_ENHANCEMENTS.md` | 위 백로그에서 갈라 나온 **출시 이후 고도화 항목**(2026-08-30 신설, **현재 활성 일감 목록**). 완료: #43~#46(1회권 소모 결함 둘 + 홈 카드 순서·첫돌이 문구) — 발행 당일 출시 전으로 당겨졌다. 예정: #39 돋보기 드래그 착수 · #26 프리미엄 월 구독 전환 · #18 봇 캐릭터 개별 구매(콘솔 게이트) · #32·#33 한국어 하드코딩 i18n. 표기 규칙과 착수 프로토콜은 원 백로그와 동일하다 |
