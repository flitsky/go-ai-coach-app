...
| 위치 | 용도 |
| --- | --- |
| `docs/AI_COLLABORATION_BACKLOG_METHOD.md` + `docs/knowhow-docs/_SPEC_PLAN.md` + `_SPEC_BACKLOG.md` | **AI 스레드와 협업하는 단일 문서 백로그 방법론**(2026-08-24, 설명 문서 1개 + 이식용 빈 템플릿 파일 2개로 분리). 핵심 가치 두 축 — ① 팀 상위 이슈 트래커(Jira 등) 아래 개인이 할당받은 일감을 이 문서로 추적, ② 한 사람이 여러 프로젝트를 오갈 때 맥락을 잃지 않음. 가이드 문서는 `engagement-growth/` 백로그에서 실제로 굴려본 운영 규칙 8가지, 선택적 커밋 리듬, 팀 트래커·보고 연계 방법, 겪은 마찰과 대응을 다루고, `knowhow-docs/`의 두 템플릿 파일은 그대로 복사해 새 트랙을 시작하는 실행 파일이다(구현 결정 각주 작성 공식 포함). **새 트랙을 시작하려는 사람은 `knowhow-docs/`의 두 파일을 `[기능명]_SPEC_PLAN.md`/`[기능명]_SPEC_BACKLOG.md`로 복사하면 된다.** 제품 문서가 아니라 일하는 방식 문서라 위 두 분류(핵심 문서/마스터플랜) 어디에도 속하지 않아 별도 절로 둔다 |
