...
| 위치 | 용도 |
| --- | --- |
| `docs/AI_COLLABORATION_BACKLOG_METHOD.md` | **AI 스레드와 협업하는 단일 문서 백로그 방법론**(2026-08-24 작성, 같은 날 외부 v2.0 초안을 비판적으로 검토해 개정). `engagement-growth/` 백로그에서 실제로 굴려본 방식을 정형화한 것 — 스펙/백로그 2단 분리, 고정 프롬프트, 완료·진행중·예정 3단 상태, 운영 규칙 10가지, 조직 트래커·보고 연계(선택, 아직 미실측), 겪은 마찰과 대응, 이식용 빈 템플릿, 이슈 트래커와의 비교까지. **새 트랙을 백로그 방식으로 시작하려는 사람은 이 문서의 8장 템플릿을 복사해 쓰면 된다.** 제품 문서가 아니라 일하는 방식 문서라 위 두 분류(핵심 문서/마스터플랜) 어디에도 속하지 않아 별도 절로 둔다 |
