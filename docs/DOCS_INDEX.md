...
| 위치 | 무엇인가 | 성격 | 상세 |
| --- | --- | --- | --- |
| `docs/` (최상위 15개) | 지금 제품을 이해/운영하는 데 바로 필요한 핵심 문서 + 인수인계 진입점(`HANDOVER.md`) | 요약+딥다이브, 항상 최신 | 아래 "핵심 문서" 표 |
...
| `docs/measurements/engine-match/`, `docs/measurements/engine-benchmark/`, `docs/error-cases/` | 엔진 성능·버그 재현 raw/summary 로그 | 데이터, 구조 고정 | "데이터 로그" |
...
## docs/ 최상위 핵심 문서 (15개, 2026-08-31 기준 — 아래 표가 이 색인 자신을 포함해 전수 등재)

⚠️ **구조 정책의 상한은 5개다**(위 "문서 구조 정책"). 지금은 그보다 많고, 줄이는 방향과
목표 형태는 그 절의 "현재 상태"에 적어 뒀다 — 이 표를 늘릴 때 반드시 그 절을 함께 볼 것.
...
| --- | --- | --- |
| 최상위 문서 | **15개** | 5 |
| 하위 폴더 | **8개**(로그 분리로 9 → 8) | 5 |
| 최대 깊이(문서) | **2** ✓ | 3 |
| 최대 깊이(`measurements/`) | 4 — 적용 대상 밖 | — |

로그 분리만 끝냈고 **최상위 정리와 중분류 통합은 남아 있다.** 목표 형태는 아래와 같다.

- **최상위 5개**: `HANDOVER.md`(진입점) · `PRD.md` · `ARCHITECTURE.md` · `ENGINE.md` · `OPERATIONS.md`
  — 이 인덱스가 이미 나눠 둔 "요약 4개 + 딥다이브" 구조를 그대로 쓴 것이다. 딥다이브
  (`ENGINE_API_CALL_POLICY` · `USER_OPTION_MANUAL` · `DIAGNOSTIC_EVENT_SCHEMA` ·
  `SCORE_AND_ENDGAME_DECISION` · `ATTENDANCE_REWARD_POLICY` · `APP_IA_AND_UI_SPEC` ·
  `GO_AI_COACH_ARCHITECTURE_ROADMAP`)는 하위 폴더로 내린다.
- **중분류 5개**: `architecture/` · `engine/` · `measurements/` · `roadmap/` · `history/`
  (현재 8개 중 `engine-research`→`engine/`, `error-cases`→`engine/` 또는 `history/`,
  `knowhow-docs`·`market-listing-history`→`history/`, `refactoring`→`roadmap/`으로 흡수)
