...
- `Top Moves` 후보 표시와 착수 후 돌 중앙에 남는 평가 점은 색상 정책이 다르다.
  ...
  - `Top Moves` 후보 표시는 추천 후보를 고르는 화면이므로, 표시된 점수 후보 전체가 큰 손실로 잡히는 희소/저예산 분석 상황에서는 후보끼리 상대 보정한다. 이때 가장 덜 손실인 후보는 연두색, 가까운 후보는 노랑, 더 멀어지는 후보는 주황/빨강으로 표시한다.
  - 이 보정은 “나쁜 수를 좋은 수로 평가한다”는 뜻이 아니라, 현재 저예산 분석에서 확보된 후보 중 어떤 수가 상대적으로 나은지 보여주기 위한 UI 표시 정책이다.
