...
- `초급`: 1~7단계 모두 Beginner 32 visits / 500ms. 같은 엔진 요청을 보내고, 1~6단계는 후보 선택 구간을 다르게 섞으며, 7단계는 초급 B32 경계 안에서 최적수만 선택한다.
- `중급`: Casual 64 visits / 500ms. 일반 대국과 후보 품질의 절충 설정이다.
- `고급`: Intermediate 160 visits / 1000ms. 더 강한 응수와 분석 품질을 목표로 한다.
- AI 응수는 대국 속도를 위해 Top Moves용 policy/refine 보강을 붙이지 않는다. 예를 들어 `중급 5단계`는 `64 visits / 500ms` 요청에서 최적수만 선택한다.
