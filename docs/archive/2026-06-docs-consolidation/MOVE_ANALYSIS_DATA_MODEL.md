...
2026-06-10 현재 모바일 기본 정책은 성능을 우선한다. 사람 차례가 오면 착수 리뷰를 위해 best-1 경량 분석 snapshot을 백그라운드로 만들고, `Top Moves` 토글이 켜져 있을 때만 같은 snapshot을 보드에 표시한다. 전체 합법 착점 coverage, policy 후보, refine sweep은 향후 broad study analysis로 분리한다.
...
- 현재 모바일 기본 보드 시각화는 `Top Moves`가 켜져 있을 때 `Scored` best-1 착점만 그린다.
- 착수 리뷰는 snapshot에서 실제 착수 좌표를 조회한다. 모바일 경량 snapshot은 best-1만 채우므로, 사용자가 후보에 없는 수를 두었거나 착점에 `pointLoss`가 없으면 실수로 단정하지 않고 `unknown`으로 남긴다.

## 현재 모바일 기본 흐름

현재 대국 중 경량 힌트/AI 응수는 broad JSON analysis를 사용하지 않는다.

1. 사람 차례가 오면 현재 Player Setup의 visits/time으로 `candidateCount=1` 분석을 요청한다.
2. `includePolicy=false`, `refinePolicyMoves=0`, `minVisitsPerCandidate=0`, `minTimeMillis=null`을 사용한다.
3. KataGo process adapter는 이 경량 요청을 JSON analysis process가 아니라 GTP `kata-search_analyze` fast path로 처리한다.
4. 응답 후보는 `Scored` best-1 후보가 되고, `MoveAnalysisSnapshot`에 저장된다.
5. `Top Moves`가 켜져 있으면 보드에는 이 후보만 표시한다. 꺼져 있으면 표시하지 않고 착수 리뷰에만 쓴다.
...
- 현재 모바일 기본은 best-1만 채우므로 여러 후보 색상, 착수별 자동 평가점, 전체 heatmap은 제한된다. 후보에 없는 착점은 `unknown` 또는 회색 계열로 처리한다.
...
5. 최신 호출 정책은 `docs/ENGINE_API_CALL_POLICY.md`를 기준으로 유지한다.
