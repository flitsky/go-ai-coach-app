...
2026-06-17 현재 `Top Moves`는 학습용 broad 후보 분석이 아니라 경량 best-5 분석이다.
...
2026-06-13 현재 모바일 기본 구현은 성능을 우선하되, 탐색 시간 상한은 사용자가 `Search Time` 메뉴에서 조정할 수 있다. 2026-09-23 기준 이 메뉴가 주는 것은 visits별 cap이 아니라 **전역 `SearchTimeLimit` 하나**이며, `Off`를 선택하면 `AnalysisLimit.timeMillis=null`로 내려가 응답시간 제한 없이 요청 visits 충족을 우선한다.
...
- 사람 착수 리뷰: 사람 차례가 오면 fast best-5 분석을 백그라운드로 요청한다 (2026-06-17 이전에는 best-1).
- Top Moves 표시: 사용자가 토글을 켜면 같은 fast best-5 snapshot을 보드에 표시한다. 1순위는 큰 원, 2~5순위는 작은 원으로 그린다.
- Top Moves 결과 적용: 엔진 응답이 돌아왔을 때 요청 당시 position fingerprint와 `AnalysisCacheKey`가 현재 상태와 일치할 때만 화면에 적용한다. 무르기, 새 게임, 복원, search time 변경 등으로 국면이나 분석 key가 바뀐 경우 해당 응답은 stale result로 보고 적용하지 않는다.
- Score estimate 결과 적용: 사용자가 수동 점수 추정을 요청한 경우에도 요청 당시 position fingerprint와 현재 국면이 일치할 때만 점수 그래프/텍스트를 갱신한다. 현재 국면이 바뀐 뒤 도착한 과거 점수 추정 성공/실패는 stale result로 보고 적용하지 않는다.
...
| 레벨 그룹 | visits | 그룹 내부 기본 `timeMillis` | 후보 상한 | 의미 |
| --- | ---: | ---: | ---: | --- |
| 빠른 초급 | 16 | 1000ms | 8 | 폰 실시간 대국 우선 |
| 초급 | 32 | 2000ms | 16 | 더 안정적인 초급/학습 후보 |
...
| 4 | `deadStones()` | 낮음~높음 | 기본 종국 pass/pass | 사석 정리. 기본 빠른 종국은 assistant judge 2초 cap을 주입 |
| 5 | `scoreFinal()` | 낮음~매우 높음 | 기본 종국 pass/pass | KataGo 원문 최종 계가 진단. 기본 빠른 종국은 assistant judge 1초 cap을 주입 |
| 6 | JSON broad analysis, `includePolicy=true` | 높음 | 기본 경로 비활성 | 여러 후보/정책 후보 확보 |
| 7 | policy refine / sweep / deep fallback | 매우 높음 | 기본 경로 비활성 | 모든 합법 착점 평가, 복기/학습 모드 |
...
EngineAdapter.analyze(
    ...
    timeMillis = 전역 Search Time 값 (Off일 때 null),
...
`deadStones()`와 `scoreFinal()`은 엔진 코어 API로는 원시 기능을 그대로 노출하되, 앱 UX에서는 그대로 무제한 동기 실행하지 않는다. 2026-06-14 실기기 계측에서 `Time cap OFF` 상태의 같은 종국 국면이 `deadStonesMs=17873`, `finalScoreMs=28845`, `totalWithSyncMs=46964`를 기록했다. 기본 빠른 종국은 두 raw 명령을 모두 시도하되, `deadStones()` 직전 2초 cap, `scoreFinal()` 직전 1초 cap을 각각 주입한다.

운영 정책은 다음과 같다.

- 대국 중 `Search Time = OFF`라도 기본 종국 판정은 assistant judge cap을 가진다.
- 현재 구현은 `deadStones()` 직전에 `EngineProfile.analysisLimit.timeMillis=2000`, `scoreFinal()` 직전에 `timeMillis=1000`으로 `configure()`를 다시 호출해 KataGo GTP process-global `maxTime`을 단계별로 제한한다.
- 기본 화면 결과는 `deadStones()` + 로컬 사석 fallback + 로컬 계가 + 기존 NN/Top Moves 추정 조합으로 만들고, `scoreFinal()` 원문은 진단 로그에 남긴다.
- debug log에는 `assistantJudgeDeadStonesTimeCapMs=2000`, `assistantJudgeFinalScoreTimeCapMs=1000`, `assistantJudgeTotalTimeCapMs=3000`, `diagnosticKataGoFinalScore=...`를 남긴다.
- 무제한 또는 긴 제한의 주심 판정은 사용자가 `이의 제기: 주심 분석 요구`를 눌렀을 때만 실행한다. 기본 대국 흐름에서는 몰래 백그라운드 검증을 돌리지 않는다.
- 부심과 주심이 서로 다른 승패 또는 큰 점수 차이를 내면 `critical` diagnostic event로 기록한다.
- GTP 명령 timeout을 강제로 끊는 경우, 늦게 도착한 응답이 process stream을 오염시킬 수 있으므로 process restart 또는 명시적 재동기화 정책과 함께 구현한다. 주심 분석은 가능하면 별도 engine worker/process 또는 session snapshot 기반으로 수행한다.
...
3. 현재 exact cache key는 `GameState.analysisFingerprint()`, `EngineSearchMode`, `AnalysisLimit` 전체 값이다.
4. 저장 시 `createdAtMillis`, `requestedRootVisits`, `rootVisits`, `origin`, 후보 리스트, summary를 함께 기록한다.
5. 같은 key의 기존 entry보다 `rootVisits`가 낮은 결과로는 덮어쓰지 않는다.
6. root visits가 같으면 더 신뢰도가 높은 origin을 우선한다.

### Exact hit과 compatible hit

초기 구현은 정확성 우선으로 `exact-hit`만 자동 의사결정에 사용한다.

| 구분 | 조건 | 기본 사용처 |
| --- | --- | --- |
| `exact-hit` | board fingerprint, ruleset, komi, next player, model/runtime, search mode, `AnalysisLimit`이 모두 일치 | AI 착수, Top Moves, 착수 리뷰에 사용 가능 |
| `compatible-hit` | 같은 board/ruleset/komi/model/search mode이고 `rootVisits >= requiredVisits` 등 품질 범위가 충분하지만 time cap, candidate count, includePolicy 같은 세부 budget이 완전히 같지는 않음 | 후속 정책. 우선 표시/추천 보조 후보로 검토 |
| `diagnostic-hit` | 같은 국면의 참고 자료는 있으나 root visits나 옵션 호환성이 부족 | 자동 의사결정 금지. 로그/진단용 |

`AnalysisLimit` 전체를 key에 넣으면 작은 time cap 변경이나 candidate count 변경만으로 cache hit이 사라진다. 이는 정확성은 높지만 hit rate를 낮춘다. 따라서 장기적으로는 visits 범위 정규화와 cache 품질 등급을 조합한다.

후속 compatible-hit 설계 원칙:

- `rootVisits >= requiredVisits`이면 visits 관점에서는 상위 품질로 볼 수 있다.
- 단, model version, ruleset, komi, board fingerprint, next player, search mode가 다르면 compatible-hit으로도 쓰지 않는다.
- `includePolicy`, `refinePolicyMoves`, candidate count가 다른 경우는 후보 표시에는 일부 재사용 가능하더라도 AI 착수 결정에는 신중하게 제한한다.
- compatible-hit을 사용한 결과는 Copy Log와 runtime log에 `cacheHit=compatible`로 남겨 exact-hit과 구분한다.
- 운영자 제공 cache는 exact-hit과 compatible-hit 모두 origin과 품질 등급을 함께 표시해, 나중에 정책을 바꿔도 원본 데이터를 재평가할 수 있게 한다.
...
| Origin | 의미 | 현재 상태 |
| --- | --- | --- |
| `local-user` | 사용자가 자신의 기기에서 플레이/최적화하며 생성한 cache | 구현 |
| `bundled-trusted` | 앱 APK/AAB에 기본 탑재되는 신뢰 cache | 도메인 준비, 데이터 탑재는 후속 |
| `operator-trusted` | 운영자가 검수해 원격 업데이트로 배포하는 cache | 도메인 준비, 다운로드/검증은 후속 |
| `peer-shared` | 원격 유저 간 루트비짓 충족 데이터를 교환한 cache | 도메인 준비, 교환 프로토콜은 후속 |

운영자/앱 기본 제공 cache는 초반 10수의 검증된 opening 데이터를 우선 목표로 한다. 유저 로컬 cache는 실제 플레이에서 생기는 랜덤한 국면을 저장한다. 원격 유저 간 교환은 양쪽 모두 `complete` 또는 운영 정책상 충분한 root visits를 증명할 수 있을 때만 받아들이는 방향으로 설계한다.
...
실제 플레이 cache key는 유지하되 실행 limit에서는 `timeMillis=null`을 사용한다. 즉 다음 플레이의 동일 budget key로 cache hit이 나도록 저장하면서, 최적화 실행 자체는 maxVisits를 채우는 쪽을 우선한다. 사용자가 `Search Time > Off`를 선택한 경우에는 실제 플레이 budget 자체도 `timeMillis=null`이므로, JSON position analysis cache key와 실행 limit이 모두 uncapped 상태가 된다. 이 모드는 응답이 느릴 수 있지만 complete/partial cache를 빠르게 축적하는 데 유리하다.
...
ENGINE_PHONE_SEARCH_MODE_BENCHMARK_OUT=docs/engine/measurements/engine-benchmark/search-mode-phone-YYYYMMDD \
...
python3 scripts/run-katago-search-mode-benchmark.py \
  ...
  --out-dir docs/engine/measurements/engine-benchmark/search-mode-phone-YYYYMMDD
```
...
현재 startup benchmark는 실제 대국 속도를 그대로 예측하는 기능이 아니다. 기기 진단을 위해 B16/B32/B64를 모두 `timeCapMs=5000`으로 측정한다. 반면 실제 대국은 `Search Time`에서 사용자가 고른 **전역 time cap 하나**를 visits와 무관하게 그대로 사용한다(2026-09-23 기준 기본 10000ms).
...
| 설정 | 기본값 | 선택지 |
| --- | ---: | --- |
| `SearchTimeLimit` (전역 1개) | 10000ms (`WithinTenSeconds`) | `Off`(제한 없음), 1000ms, 3000ms, 5000ms, 10000ms |

⚠️ 기본값은 **2026-09-22에 3초에서 10초로 올렸다**(커밋 `19047906`). 이미 값을 고른 사용자는 저장된 값이 이긴다. 또 `engineTurnWatchdogTimeoutMillisFor`가 이 값에서 계산되므로 기본값을 내리면 「엔진 응답 지연」 팝업도 그만큼 빨리 뜬다.
...
값 해석 기준의 과거 상세 근거(`TOP_MOVES_VALUE_GUIDE.md`, `ENGINE_ANALYSIS_CONSISTENCY_REVIEW.md`, `GREEN_SPOT_HINT_DECISION.md`, `KATRAIN_TOP_MOVES_ANALYSIS.md`)는 2026-08-17 문서 정리에서 저장소 보관을 중단했다(`DOCS_INDEX.md`의 "문서 보존 정책" 참고). 이 문서(`ENGINE_API_CALL_POLICY.md`)가 그 결론을 이미 흡수한 현재 기준이므로, 과거 근거 원문이 필요하면 `git log --all --diff-filter=D -- 'docs/archive/**/TOP_MOVES_VALUE_GUIDE.md'`처럼 파일명으로 삭제 커밋을 찾아 복원한다.
