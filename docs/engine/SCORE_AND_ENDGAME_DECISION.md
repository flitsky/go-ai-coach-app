...
부심 판정은 사용자가 오래 기다리지 않도록 빠르게 보여주는 기본 종료 결과다. 주심 판정은 사용자가 명시적으로 요구한 정밀 분석이다. 따라서 두 결과가 크게 다를 때 앱이 조용히 부심 결과를 덮어쓰면 신뢰가 깨진다.

불일치 처리 원칙:

- 부심 결과는 `빠른 계가` 또는 `임시 판정`으로 보관한다.
- 주심 결과는 `정밀 계가`로 별도 보관한다.
- 승패가 뒤집히거나 점수 차이가 임계값 이상이면 `critical disagreement`로 기록한다.
- 화면에는 “정밀 분석 결과가 빠른 계가와 다릅니다”처럼 차이가 발생했다는 사실을 명시한다.
- 앱은 자동으로 기보 최종 결과를 덮어쓰지 않는다. 사용자가 빠른 계가 또는 정밀 계가 중 어떤 결과를 최종 채택할지 선택하게 한다.
- 선택 전에는 debug report에 두 결과, 사용한 엔진 설정, board fingerprint, session generation id, elapsed time을 모두 남긴다.

이 UX는 까다롭지만 필수다. 종국 결과는 학습 기록과 기보 신뢰도에 직접 영향을 주므로, 앱이 “나중에 다른 결과가 나왔으니 몰래 바꿨다”는 인상을 주면 안 된다.
...
- `docs/engine/error-cases/pass-pass-dead-stones-g2-h2.md`
...
- `docs/engine/error-cases/pass-before-cleanup-top-move-flip.md`
...
- `docs/engine/error-cases/pass-after-white-pass-score-flip-20260607.md`
