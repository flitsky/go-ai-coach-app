...
- `GameSessionStateHolder`(5계층)는 여전히 `app-android`에 있다. `shared`로 옮기는 KMP 이식은 아직 안 함 — 이식 가능성 자체(플랫폼 비종속)는 `engineOperationApplicationPoliciesStayPortable`이 이미 `application/` 트리 전체를 검증하고 있어 확인된 상태다. **260816**: `application/safety/` 스파이크(아래 "고도화 로드맵" 5번)로 이전 절차 자체(물리 이동·`internal`→public·JUnit→kotlin.test·양쪽 모듈 컴파일·iOS 타깃 컴파일까지)를 실제로 검증 완료 — 전부 문제없이 통과했다. 같은 날 본 이전도 착수해 웨이브 1(56개 파일)을 완료(진행 중, 남은 68개 파일은 웨이브 2~6) — 상세는 아래 "고도화 로드맵" 5번 및 `docs/refactoring/GAMESESSION_SHARED_MIGRATION_KICKOFF_PLAN_260816_1808.md`.
...
5. **5계층 — `GameSessionStateHolder` → `:shared` KMP 이전.** 이식 가능성(플랫폼 비종속)은 `engineOperationApplicationPoliciesStayPortable`(125개 파일, 21개 서브패키지 전체 스캔)이 이미 검증. `GameSessionCoreState`/`GameSessionController`가 `autoai/engine/humanmove/savedgame/score/startgame/topmoves/undo/debugreport/analysis/movereview/preferences` 12개 서브패키지를 전이적으로 끌어들여 "일부만 이전"이 불가능 — 사실상 `application/`(125개 파일) 전체가 한 단위.
   ...
   - **(2) 본 이전(진행 중 — 웨이브 1/6 완료, 260816)**: 착수 계획서 `docs/refactoring/GAMESESSION_SHARED_MIGRATION_KICKOFF_PLAN_260816_1808.md`가 나머지 124개 프로덕션 파일(+기존 단위테스트 50개)의 실제 파일 단위 의존 그래프를 스캔해 이동 순서를 확정했다. **계획 수립 중 정정한 예상**: "`internal`→기본 가시성 확대는 실제 app-android 호출부가 있는 지점만"이라고 예상했었는데, 스파이크(파일 1개) 결과를 트리 전체로 낙관적으로 외삽한 것이었다 — 실측하니 509개 중 424개(83%)가 이미 `ui/`/`persistence/`/`middleware/`/테스트에서 참조되고 있어 이동 순서와 무관하게 public 전환이 필요했다. **같은 날 이어서 웨이브 1(56개 파일, 원래 계획한 19개가 아니라 파일 단위 의존 그래프 조사 자체의 결함 — 같은 패키지 내부의 무-import 참조 누락 — 을 실행 중 발견/교정한 뒤 확정된 진짜 원자적 단위) 실행까지 완료**: `:shared`/`:app-android` 컴파일 + `make test` 전체 그린. `internal`→public 전환은 사전 감사가 아니라 컴파일러가 지목하는 대로 반복 수렴하는 방식이 실제로 잘 작동했고, `LayeringContractTest.kt`의 하드코딩 경로 문제(`applicationFile()` 헬퍼로 해결)와 Kotlin 크로스모듈 스마트캐스트 제약(로컬 val 캡처로 해결)이라는 두 가지 새 함정도 발견/처리했다 — 상세 경위와 남은 68개 파일의 웨이브 2~6 계획은 착수 계획서 0·3·7·9절 참고.
   - **(3) `LayeringContractTest.kt` 갱신**: 항목 8과 동일하게, 본 이전이 실제로 끝난 뒤에만.
   - **(4) 회귀 확인**: `NewGameBoardTapSmokeTest.kt`/`AppLaunchSmokeTest.kt`.
