...
2026-06-14 실기기 로그에서 `Time cap OFF` 상태의 양패스 종국 처리 중 `final_status_list dead`가 약 17.9초, `final_score`가 약 28.8초 걸린 사례가 재현됐다. 일반 착수 분석은 2초 안팎으로 끝나더라도, GTP 종국 명령은 AI 착수 분석의 `visits/timeMillis` 제약과 별도로 동작하므로 사용자에게 1분 가까이 대기를 요구할 수 있다.

따라서 종국 UX는 다음 2심판 구조를 목표로 한다.

- 부심 판정: 사용자 흐름을 막지 않는 빠른 판정이다. 로컬 사석 fallback, 이미 확보된 Top Moves/NN estimate, 필요 시 짧은 제한의 엔진 사석 판정을 조합해 2~10초 SLA 안에서 화면 결과를 먼저 표시한다.
- 주심 판정: 시간 제한 없이 또는 더 긴 제한으로 실행하는 정밀 검증이다. 기본 UX를 막지 않고 백그라운드 진단으로 수행한다.
...
이 정책의 핵심은 사용자가 보는 최종 화면을 빠르게 안정화하고, 긴 엔진 검증은 품질 감시와 오류 수집 용도로 분리하는 것이다.
