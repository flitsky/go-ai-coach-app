...
**계약이 아는 것 — 2026-09-24(refactor backlog #73)**: 원칙 문서가 3계층 경계 원칙도 4계층과 같은 어휘(*"위 계층의 동작은 모르고, 값과 순수 규칙만 안다"*)로 고쳤다. 5계층이 보는 `EngineSessionClient`는 시그니처 타입 23개의 필드 폐포 52개 타입이 전부 값이다. 벗어나는 것은 하나 — `runStartupBenchmark(onProgress: suspend (EngineBenchmarkProgress) -> Unit)`의 `onProgress`는 4계층 규칙 ⓐ가 금지하는 동작 콜백이다(값의 흐름으로 돌려주면 풀린다). 구현 쪽(ⓑ)과 나머지 3계층 계약(`AiMoveEngineGateway`·`EndgameJudgeGateway` 등)은 이번에 재지 않았다.
...
**포트가 아는 것 — 2026-09-24 실측(refactor backlog #73)**: 원칙 문서가 옛 문장 *"5계층 이상을 모른다"* 를 이 실측에 맞춰 규칙 둘로 고쳤다 — ⓐ 포트 **시그니처**는 값만 싣는다(값 = 필드 폐포에 함수·인터페이스·`var`·포트/클라이언트가 없는 것), ⓑ 어댑터 **구현**은 위 계층의 순수 규칙을 부르되 판정식을 갖지 않는다(`docs/ARCHITECTURE.md` 4계층 "포트가 아는 것"). 대상은 `*Port` 인터페이스 **19개**와 이름이 Port가 아닌 `PositionAnalysisCacheStore` 1개, 프로덕션 어댑터는 계약마다 하나씩 **20개**다. ⚠️ 1차(`efb0ab38`)는 둘을 한 문장(*"동작을 모른다"*)에 섞고 값을 *"데이터 클래스"* 라는 선언 종류로 정의해서, 아래 ⓑ 허용 사례가 있는 지금 코드에서 거짓이었고 클라이언트·람다를 담은 `*RunRequest`도 값으로 통과시켰다 — 검수 반려로 다시 썼다.

- **ⓐ 시그니처 타입 — 위반 0.** 13개가 자기가 저장·전달하는 5·6계층 값 타입을 싣는다. import로 보이는 것은 5개(`AuthClientPort`·`PremiumStateStorePort`·`DiagnosticEventLogPort`·`DiagnosticEventExternalSinkPort`·`BoardVisionScannerPort`)뿐이고, 나머지 8개는 값과 **같은 패키지**라 import 줄이 없다 — `#29`가 드러낸 "둘"은 보이는 끝이었다. 시그니처 타입 23개에서 필드 폐포를 풀면 **69개 타입**(data class 44·data object 1·enum 21·값 sealed interface 3 — `PurchaseOutcome`·`AdRewardOutcome`·`Move`, 하위 타입까지)이고, 함수 타입 필드·인터페이스 타입 필드·`var`·가변/반응형 컨테이너·포트/클라이언트/컨트롤러 필드는 **0**이다. 포트 메서드의 함수 타입 매개변수와 `Flow`도 **0**이다 — 들어올 예정인 `AttendanceStorePort.update(transform: (AttendanceState) -> AttendanceState?)`가 첫 함수 타입 매개변수이고, ⓐ가 허용하는 순수 변환이다. 기본값 식 둘(`RuntimeEventLogPort`·`DiagnosticEventLogPort`의 `nowMillis = currentEpochMillis()`)은 규칙 밖이다. 폐포 검사의 음성 대조로 `TopMoveAnalysisRunRequest`를 넣으면 함수 타입 필드 12개·인터페이스 타입 필드 1개(`EngineSessionClient`)로 걸린다.
- **ⓑ 어댑터 — 위 계층 동작 호출 0**(20개 모두 자기 포트·값·순수 규칙만 import한다). 위 계층의 순수 규칙을 **부르기만 하는** 허용 사례: `PremiumStateStore.load`→`PremiumState.isClockPlausibleAt`, `GameSessionStore`→`SavedGameSnapshot.isResumable`·`GameStateReplayer.replay`, `GameHistoryStore`→`GameHistoryRetentionPolicy.idsToEvict`, `UserPreferencesStore`→`migrateSettingsSchema`·`sanitizeAppFontScale`·`SearchTimeSettings.normalized`, `JsonPositionAnalysisCacheStore`→`PositionAnalysisCacheEntry.isExpired`·`quality.isReusable`/`isStorable`·`shouldReplacePositionAnalysisCacheEntry`, 세 저장소가 함께 쓰는 `PlayerSetupJsonCodec`→`PlayLevelSetting.normalized`. *"쓸 만한 것만"* 돌려주는 포트 메서드 중 `PremiumStateStorePort.load`(믿을 수 없으면 기본값)·`PositionAnalysisCacheStore.get`/`peek`(만료·재사용 불가 제외)는 판정식이 위에 있어 **허용**이고, `SavedGameStorePort.load`는 아래 ②, 판정 자체를 묻는 `hasUsableProfile`은 아래 ①이다.
- **위반 3 — 판정식이 어댑터 안에만 있거나(판정 요구 메서드 기준), 위에 있는 것을 다시 적었다(ⓑ).** ① `EngineBenchmarkStorePort.hasUsableProfile` — 판정식(샘플 수·시간 상한·측정 버전·기준 규칙 일치, 목표 방문 수마다 표본 수 충족)이 `persistence/EngineBenchmarkStore.kt` 안에만 있고 위 계층에 같은 식이 없다. 프로덕션 호출부 0(`#88`). ② `GameSessionStore`의 보관 조건 `isResumable || finalScoreJudgement != null`(save·load 두 곳) — 핵심은 위의 `isResumable`을 부르지만, *"판정 결과가 있으면 끝난 판도 남긴다"* 는 예외를 어댑터가 덧붙이고, 같은 예외가 위 `planSavedGamePersistence`에 다른 식으로 적혀 있다(복제). ③ `JsonPositionAnalysisCacheStore.put`의 `searchMode != JsonPositionAnalysis` 거르기 — 위 `LocalPositionAnalysisCacheCoordinator.storeIfEligible`의 같은 조건을 다시 적었다(복제).
- **판정이 아닌 것**(어댑터에 남아도 된다): 스키마 번호·파싱 실패·없는 키의 기본값 흡수, 0개·빈 id·빈 한 줄 평을 "없음"으로 접기, SDK 응답 코드 해석(결제·로그인·광고), `TrimmedAppendOnlyLog`의 로그 바이트 상한, JSON 캐시를 상한까지 최신순으로 자르기(상한 값 `JsonPositionAnalysisCacheMaxEntries`는 위 계층 상수다).
- **`PositionAnalysisCacheStore`는 3계층 계약으로 본다** — 모델 옆(`application.analysis`)에 있고 소비자도 3계층 `PositionAnalysisCacheResolver`뿐이라 *"3계층 캐시가 자기 어휘로 적은 저장 요구"* 이고, 구현 `JsonPositionAnalysisCacheStore`만 4계층 기기 저장 어댑터다. 그래서 3계층은 4계층을 모르는 채로 남지만, 어댑터 쪽(4→3)에서 *"3·4계층은 서로 모른다"* 가 한 번 건너진다 — 원칙 문서 기준으로 갭이다. ⓐ·ⓑ는 다른 포트와 똑같이 적용하고(위 수치에 포함돼 있다), 가드는 이름으로 못 잡으니 명시 등록한다.

⚠️ **이 규칙을 보는 가드는 없다.** `LayeringContractTest`의 포트 쪽 가드(`authPremiumAndDeviceApplicationPackagesStayPlatformFree`·`engineOperationApplicationPoliciesStayPortable`)는 아래쪽 절반(플랫폼 API 금지)만 보고, `PackageCycleRatchetTest`는 사이클만 본다. 옛 정의도 검사된 적이 없다 — 검사됐다면 `#29`의 import 두 줄이 빨갰을 것이다. 가드 신설은 `#83`이고, 위 수치는 그 가드의 첫 기준선이다. ⓑ의 *"판정식을 어댑터에 쓰지 않는다"* 는 기계로 잡기 어려워 리뷰가 맡는다.
