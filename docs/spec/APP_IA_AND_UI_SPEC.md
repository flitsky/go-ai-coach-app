...
graph TD
    ...
    C --> D["햄버거 메뉴 다이얼로그 (제목: 설정)"]
    D --> D1["언어"]
    D --> D2["Search Time (탐색 시간)"]
    D --> D3["Display Options (좌표·수순·돋보기·햅틱·직접 착수)"]
    D --> D4["진단 (로그 복사 · 엔진 벤치마크)"]
...
- **핵심 UI 요소**:
  ...
  5. **햄버거 메뉴 다이얼로그** (`ui/GameMenuSection.kt`의 `ExpandedGameMenuSection`, 제목: **`설정`**):
     - `언어`, `Display Options`(좌표·수순 번호·최근 수 표시·돋보기 크기/배율·햅틱·직접 착수·매수 평가/추천수·검토 모드), `Search Time (탐색 시간)`, 진단 액션(로그 복사 / 엔진 벤치마크)을 실시간 변경·실행.
