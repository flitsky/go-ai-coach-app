...
### 2.5. 바둑 규칙 배우기 (`StudyRulesScreen`) — 백로그 #164

- **목적**: 처음 두는 사람이 **한 판도 두기 전에** 읽는 규칙. 학습 허브의 첫 콘텐츠다.
- **형식**은 ⓐ+(2026-09-22 사용자 결정, U-17) — **정적 문구 + 도해 넘김.** 문제 풀이·정오 판정·
  진행도 저장소는 **없다**(그 셋은 「기본 사활」의 몫이다).
- **단원 여섯**(순서가 곧 화면의 순서다): ① 바둑판과 첫 수 ② 활로 ③ 단수와 따냄
  ④ 둘 수 없는 자리 ⑤ 패 ⑥ 집과 계가. 단원마다 3~6장의 도해를 「이전 / N·M / 다음」으로 넘긴다.
  마지막 장의 오른쪽 버튼은 **「다음 단원」**(마지막 단원에서는 「단원 목록으로」)이다.
...
- ⚠️ **콘텐츠는 바이너리에 못박혀 있다**(U-19, 2026-09-22) — 판 위의 사실은 `shared`의
  `GoRuleLessons.kt`(KMP), 네 언어 문구는 `UiStringsStudyRules.kt`의 곁표다. 원격 배포를 열면
