...
class UiStringsTest {
     ...
     * `appTitle`은 언어마다 다른 값을 갖는데(`포켓 바둑 코치`/`Go AI Coach`/`囲碁AIコーチ`/`围棋AI教练`)
     * 셋 다 한글이 없으니 그냥 통과한다. ⚠️ 2026-09-06까지 이 주석은 *"브랜드명이라 세 언어가 일부러 상속한다"*
     ...
     *   ⚠️ **이 규칙은 구독으로 바뀐 뒤에도 그대로 유효하다**(2026-09-22 실제로 한 번 걸렸다).
     *   문구가 구독 이야기가 되면서 *"자동으로 복원된다"* 로 적었다가 이 단언에 잡혔다 —
     *   **구독은 만료·강등이 있어** 무조건적 복원을 약속하면 해지한 사용자에게 거짓이 된다.
     *   지금 문구는 *"앱이 구독을 **확인**해 모든 기능을 다시 열어 드립니다"* 로, 조건(같은
     *   Google Play 계정·구독 중)이 문장 안에 있다.
