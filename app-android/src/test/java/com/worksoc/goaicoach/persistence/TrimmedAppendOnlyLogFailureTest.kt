 ...
 * 삼키고, 기록이 실패한 횟수는 [TrimmedAppendOnlyLog.readText] 끝줄에 밝힌다 — 조용히 사라지지 않게,
 * 그리고 같은 파일을 새 인스턴스로 여는 진단 로그 화면에서도 보이게(파일 경로 단위로 센다).
 */
class TrimmedAppendOnlyLogFailureTest {
    ...
    fun runtimeLogAppendDoesNotThrowWhenTheMediumFails() {
        ...
        assertTrue(text, text.contains("${TrimmedAppendOnlyLog.MediumFailureNotePrefix}2"))
    }
    ...
    fun diagnosticLogAppendDoesNotThrowWhenTheMediumFails() {
        ...
        val text = log.readText()
        assertTrue(text, text.contains("${TrimmedAppendOnlyLog.MediumFailureNotePrefix}1"))
    }
    ...
    fun clearResetsTheFailureCount() {
        ...
        assertTrue(!log.readText().contains(TrimmedAppendOnlyLog.MediumFailureNotePrefix))
    }

    @Test
    fun healthyLogReportsNoMediumFailures() {
