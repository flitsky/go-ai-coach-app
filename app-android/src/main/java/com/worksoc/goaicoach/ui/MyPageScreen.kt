...
internal fun MyPageScreen(
    ...
    if (ShowUserProfileRow && isEditingNickname) {
        UserNicknameDialog(
    ...
    Column(
        ...
        Column(
            ...
            //
            // ⚠️ **자리를 최상단으로 고른 이유**: 이 화면이 나열하는 것들(출석 도장판·1회권) 중
            // 구독만이 **매달 돈이 나가는 항목**이고, 해지 경로는 *찾기 어려우면 그 자체가
            // 정책 문제*가 된다. 구독자에게는 업셀 팝업이 뜨지 않으므로 앱 안에서 해지에
            // 닿는 길이 여기 말고는 없다.
            // ⚠️ **그럼에도 1.5줄이다** — 이 화면의 성격은 *"내가 모은 것"* 이지 판매처가
            //   아니다(이 파일 KDoc). 카드가 두꺼워지면 그 성격이 바뀐다.
            // ⚠️ 카드는 **아래 인사와 한 덩어리가 아니다** — 2026-09-09 판정이 묶은 것은
            //   *인사와 출석 제목*이고, 그 덩어리 위에 독립된 카드가 한 장 오는 것은 그
            //   판정이 풀려던 "세 덩어리가 얽혀 보인다"를 다시 만들지 않는다.
            // ⚠️ 상태는 카드가 스스로 든다 — 이 화면이 `GoCoachApp.kt`에 상태를 두지 않는
            //   것과 같은 이유다(그 파일은 상태 훅 여유가 0이다).
            // **「나」 줄**(백로그 #165) — **지금은 꺼 둔다**([ShowUserProfileRow]).
            //
            // ⚠️ **켤 때의 자리는 여기, 구독 카드 위다**(2026-09-22 사용자 지시). 그 순서를
            // 지키는 이유는 아래 구독 카드의 주석에 있다 — 켜면서 자리를 바꾸지 말 것.
            if (ShowUserProfileRow) {
                UserProfileRow(
                    nickname = nickname,
            ...
            // **프리미엄 구독 카드**(백로그 #159, 2026-09-18 사용자 확정).
            //
            // ⚠️ **해지에 닿는 길이 앱 안에서 여기뿐이다** — 구독자에게는 업셀 팝업이 뜨지 않는다.
            // 그래서 이 카드는 **스크롤 없이 보여야 한다.** 2026-09-18이 이 카드를 맨 위에 둔
            // 사유가 그것이고, 2026-09-22에 「나」 줄이 그 위로 올라갈 수 있었던 것도 그 줄이
            // 한 줄짜리라 카드가 여전히 첫 화면에 남았기 때문이다(실기 확인).
            // ⚠️ **둘 중 하나라도 두꺼워지면 그 전제가 깨진다** — 실기로 다시 볼 것.
            // ⚠️ **그럼에도 1.5줄이다** — 이 화면의 성격은 *"내가 모은 것"* 이지 판매처가
            //   아니다(이 파일 KDoc). 카드가 두꺼워지면 그 성격이 바뀐다.
            // ⚠️ 상태는 카드가 스스로 든다 — 이 화면이 `GoCoachApp.kt`에 상태를 두지 않는
            //   것과 같은 이유다(그 파일은 상태 훅 여유가 0이다).
...
/**
 * 마이 페이지에 「나」 줄(아바타·닉네임)을 그릴 것인가 — **2026-09-22 현재 꺼져 있다.**
 *
 * ## ⚠️ 기능이 덜 됐기 때문이 아니다 — **쓸 데가 없기 때문이다**
 * #165는 닉네임을 만들고 노출을 **마이 페이지 안으로만** 한정했다(U-21). 그 결과 *"이름을
 * 지을 수는 있는데 아무 데서도 안 쓰이는"* 상태가 됐고, 사용자가 그 점을 지적했다
 * (2026-09-22): *"닉네임 쓸 일이 없어서 당장 노출하는 게 맞는가 의문"*.
 * **코드는 전부 남겨 두고 노출만 끈다** — 지우면 되살릴 때 처음부터 다시 만들어야 한다.
 *
 * ## 켜는 조건 — 이름이 **일을 하게 된 뒤**
 * 가장 값싸고 값진 자리는 **대국 좌석 카드**다(지금은 「흑 (유저)」 — 상대는 이름이 있는데
 * 나만 「유저」인 비대칭). 그다음이 계가 결과 팝업이다. 백로그 **U-53**이 그 결정을 든다.
 *
 * ⚠️ **켤 때 자리를 바꾸지 말 것** — 구독 카드 위가 2026-09-22 사용자 지시다.
 * ⚠️ 저장된 닉네임은 **지우지 않는다** — 껐다고 지우면, 켰을 때 이미 지어 둔 이름이 사라진다.
 */
private const val ShowUserProfileRow = false
