...
internal fun MyPageScreen(
    ...
    Column(
        ...
        Column(
            ...
            // **「나」 줄이 맨 위다**(백로그 #165, 2026-09-22 사용자 지시).
            //
            // ⚠️ **이 자리는 2026-09-18에 구독 카드가 받았던 것이다**(#159) — 사유는 *"구독만이
            // 매달 돈이 나가는 항목이고, 해지 경로는 찾기 어려우면 그 자체가 정책 문제"* 였다.
            // 그 사유는 **지금도 유효하다.** 다만 지키려던 것은 *맨 위*가 아니라 **「스크롤 없이
            // 닿는다」** 였고, 「나」 줄이 한 줄짜리라 구독 카드는 여전히 첫 화면에 그대로 보인다
            // — 실기로 확인한 뒤 옮겼다.
            // ⚠️ **카드가 두꺼워지거나 「나」 줄이 절로 커지면 그 전제가 깨진다.** 둘 중 하나라도
            // 자라면 구독 카드가 접힘선 아래로 내려가는지 **실기로 다시 볼 것.**
            UserProfileRow(
                nickname = nickname,
            ...
            // 그래서 이 카드는 **스크롤 없이 보여야 한다**(위 「나」 줄의 주석 참고).
            // ⚠️ **그럼에도 1.5줄이다** — 이 화면의 성격은 *"내가 모은 것"* 이지 판매처가
            //   아니다(이 파일 KDoc). 카드가 두꺼워지면 그 성격이 바뀐다.
            // ⚠️ 상태는 카드가 스스로 든다 — 이 화면이 `GoCoachApp.kt`에 상태를 두지 않는
            //   것과 같은 이유다(그 파일은 상태 훅 여유가 0이다).
