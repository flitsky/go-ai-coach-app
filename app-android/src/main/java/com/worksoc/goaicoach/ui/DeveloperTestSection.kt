...
internal fun DeveloperTestSection(
    ...
    Column(
        ...
        // 있다(#47, `GOOGLE_PLAY_LAUNCH_PLAN.md` §0 B-3의 808 vs 810). 버전만으로는
        ...
        if (BuildConfig.DEBUG && isAdvancedDeveloperModeEnabled) {
            ...
            // **앱을 최초 실행 상태로 되돌린다**(2026-09-09 사용자 요청).
            // ⚠️ **릴리즈 초기화(바로 위)와 다르다 — 더 넓다.** 그쪽은 권한 넷만 밀고 대국 기록·
            // 설정·언어·온보딩 완료를 **일부러 남긴다**. 이 컨트롤은 그 남긴 것들까지 밀어
            // **첫 실행 처리와 가이드가 다시 도는 상태**를 만들고 앱을 재시작한다.
            // ⚠️ 확인 다이얼로그 상태는 **저쪽이 소유한다** — 이 섹션의 훅 예산은 여유 0이고,
            // 그 상태는 이 섹션이 알 이유가 없다(`LayeringContractTest`가 실제로 걷어찼다).
