...
/**
 * 「바둑 규칙 배우기」의 문구 전문(백로그 #164). 구조는 `UiStringsStudyCategories.kt`와 같다 —
 * 키로 네 언어를 든다.
 *
 ...
 */
private val GoRuleLessonTitles: Map<GoRuleLessonId, Map<UiLanguage, String>> = mapOf(
    GoRuleLessonId.Basics to mapOf(
    ...
    GoRuleLessonId.Forbidden to mapOf(
...
private val GoRuleLessonSummaries: Map<GoRuleLessonId, Map<UiLanguage, String>> = mapOf(
    GoRuleLessonId.Basics to mapOf(
...
/**
 * 단계 본문 — 키는 `GoRuleStep.id`다. 좌표가 아니라 짧은 이름을 쓰는 이유는 도해를 다듬어도
 * 문구를 그대로 두는 경우가 흔하기 때문이다(`StudyVideoEntry.id`와 같은 판단).
 */
private val GoRuleStepBodies: Map<String, Map<UiLanguage, String>> = mapOf(
    "basics.points" to mapOf(
...
private val StudyRulesPrevious: Map<UiLanguage, String> = mapOf(
...
private val StudyRulesNext: Map<UiLanguage, String> = mapOf(
...
private val StudyRulesNextLesson: Map<UiLanguage, String> = mapOf(
...
private val StudyRulesBackToList: Map<UiLanguage, String> = mapOf(
 ...
 * 표를 직접 물어야 한다.
 */
internal fun goRuleLessonTitleLanguages(lesson: GoRuleLessonId): Set<UiLanguage> =
    GoRuleLessonTitles[lesson]?.filterValues { it.isNotBlank() }?.keys.orEmpty()

internal fun goRuleLessonSummaryLanguages(lesson: GoRuleLessonId): Set<UiLanguage> =
    GoRuleLessonSummaries[lesson]?.filterValues { it.isNotBlank() }?.keys.orEmpty()

internal fun goRuleStepBodyLanguages(stepId: String): Set<UiLanguage> =
    GoRuleStepBodies[stepId]?.filterValues { it.isNotBlank() }?.keys.orEmpty()

/** 표에 적힌 단계 키 전부 — 단계를 지웠는데 본문만 남는 것을 그물이 본다. */
internal fun goRuleStepBodyKeys(): Set<String> = GoRuleStepBodies.keys

/**
 * 표가 비면 키를 그대로 돌려준다 — `UiStringsStudyCategories.kt`의 폴백과 같은 이유다.
 * 빈칸이면 조용히 지나가지만, 화면에 `ko.retake`라고 뜨면 눈에 띄어 바로 고친다.
 */
internal fun goRuleLessonTitleFor(language: UiLanguage, lesson: GoRuleLessonId): String =
    GoRuleLessonTitles[lesson]?.get(language) ?: lesson.name

internal fun goRuleLessonSummaryFor(language: UiLanguage, lesson: GoRuleLessonId): String =
    GoRuleLessonSummaries[lesson]?.get(language) ?: lesson.name

internal fun goRuleStepBodyFor(language: UiLanguage, stepId: String): String =
    GoRuleStepBodies[stepId]?.get(language) ?: stepId

internal fun studyRulesPreviousFor(language: UiLanguage): String =
    StudyRulesPrevious[language] ?: StudyRulesPrevious.getValue(UiLanguage.Korean)

internal fun studyRulesNextFor(language: UiLanguage): String =
    StudyRulesNext[language] ?: StudyRulesNext.getValue(UiLanguage.Korean)

internal fun studyRulesNextLessonFor(language: UiLanguage): String =
    StudyRulesNextLesson[language] ?: StudyRulesNextLesson.getValue(UiLanguage.Korean)

internal fun studyRulesBackToListFor(language: UiLanguage): String =
    StudyRulesBackToList[language] ?: StudyRulesBackToList.getValue(UiLanguage.Korean)
