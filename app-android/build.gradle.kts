...
// ⚠️ **`DebugGeography`만으로는 아무 일도 일어나지 않는다 — 테스트 기기로 등록돼야 한다**
// (2026-09-06 실기에서 드러났다: 지오그래피를 EEA로 줬는데 `IABTCF_gdprApplies=0`이 찍혔다).
// 기기별 해시 ID는 앱을 한 번 띄운 뒤 logcat에 UMP가 직접 알려 준다:
//   `UserMessagingPlatform: Use new ConsentDebugSettings.Builder().addTestDeviceHashedId("…")`
// 그 값을 `local.properties`(gitignored)의 `consent.testDeviceHashedId`에 넣는다 — 기기마다
// 다르고 저장소에 커밋할 성질이 아니다.
val consentTestDeviceHashedId: String =
    localProperties.getProperty("consent.testDeviceHashedId").orEmpty()
...
// 개발용 원격 엔진 스파이크(work/roadmap/LAYERED_ARCHITECTURE_REFACTORING_PLAN_260803_1500.md
...
android {
    ...
    packaging {
        jniLibs {
            // ⚠️ **16KB 페이지 대응의 한 축이다 — 근거 없이 끄지 말 것**(백로그 #118, 2026-09-06에
            // 사유를 뒤늦게 적었다. 그전까지 이 줄에는 주석이 없어 "왜 켰는지" 아는 사람이 없었다).
            // `true`면 `.so`가 APK 안에 **압축된 채로** 들어가고 **설치 시 디스크로 풀린다**
            // (`android:extractNativeLibs="true"`). `false`면 APK 안에서 그대로 mmap하므로
            // **zip 정렬까지 16KB여야** 한다 — 즉 끄는 순간 요구 조건이 하나 늘어난다.
            // ⚠️ **이것만으로는 16KB 오류가 해결되지 않는다.** 811이 그 증거다 — 이 줄이 켜져 있는
            // 채로 반려됐다. **ELF 자체의 세그먼트 정렬**(`p_align=16384`)이 따로 필요하고,
            // 그건 엔진 빌드 스크립트의 `ANDROID_SUPPORT_FLEXIBLE_PAGE_SIZES=ON`이 맡는다.
            // 두 축을 하나로 착각하지 말 것.
