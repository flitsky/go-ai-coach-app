 ...
 * **#64가 잘림을 실제로 확인한 배율**이고, 1.5는 그보다 크지만 레이아웃이 아직 읽히는 상한으로
 * 사용자가 고른 값이다(2026-09-04). ⚠️ 여기에 값을 더할 때는 `FontScaleLayoutContractTest`가
 * 지키는 처방들이 그 배율에서도 버티는지 **실기로** 확인할 것 — 값만 늘리는 것은 검증이 아니다.
 */
const val DefaultAppFontScale: Float = 1.0f

/** 순환 순서. 마지막 값 다음은 처음으로 돌아온다. */
val AppFontScales: List<Float> = listOf(1.0f, 1.3f, 1.5f)
