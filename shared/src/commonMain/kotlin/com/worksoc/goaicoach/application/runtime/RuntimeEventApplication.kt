 ...
 *
 * ⚠️ **그래서 여기에 무엇을 쓸지는 판단이었고, 사용자가 `바둑 AI 코치`로 정했다**(2026-09-06,
 * #97의 "코치" 보류를 뒤집은 결정). #112에서는 *"영문 기술 산출물이니 영어 이름"* 이라는 이유로
 * `Go AI`를 넣었는데, 그러자 **문서는 `바둑 AI`, 코드는 `Go AI`** 로 갈렸다 — 이름이 둘이 되는
 * 것보다 영문 로그에 한글이 섞이는 편이 낫다는 결정이다. 리포트 제목·클립보드 라벨도 같은 값이다.
 *
 ...
 */
private const val RuntimeAppName = "바둑 AI 코치"
