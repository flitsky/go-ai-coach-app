 ...
 * 클립보드 라벨(`DebugReportBuilder.kt`)도 같은 값이다.
 *
 * ⚠️ **영문 로그에 한글 이름이 섞이는 것은 결정이다**(2026-09-06) — #112가 *"영문 산출물이니 영어
 * 이름"* 으로 `Go AI`를 넣었다가 문서와 코드가 서로 다른 이름을 말하게 됐고, 이름이 둘이 되는 것보다
 * 낫다고 봤다.
 *
 * ⚠️ **2026-09-11 앱 이름을 `포켓 바둑 코치`로 바꾸면서(#137) 이 셋을 놓쳤다** — 그때까지 셋은
 * 그물 밖이었다(런처 이름은 언어마다 달라 비교할 단일 값이 없다고 봤다). 그러나 셋 다 **한국어 이름을
 * 고정으로** 쓰므로 한국어 런처 라벨과 비교하면 된다 — 이제 `AppNameContractTest`가 셋을 거기에 묶는다.
 */
private const val RuntimeAppName = "포켓 바둑 코치"
