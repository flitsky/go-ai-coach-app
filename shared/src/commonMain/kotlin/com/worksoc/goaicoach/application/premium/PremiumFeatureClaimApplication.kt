 ...
 * ⚠️ 그 4.4절이 전제한 **출석 "자동 지급"은 이제 없다**(#14로 Claim 방식이 됐다). 그래도 이
 * 함수를 UI 밖에 둔 이유는 그대로다 — 지급을 하는 쪽이 Compose 람다가 아니라 저장소를 직접
 * 다루는 경로이기 때문이다. 이 함수는 그 판정/저장
 ...
 * 호출부가 메모리에 들고 있던 클레임 집합만 이어붙이면, 그 사이 화면 밖(출석 보상 Claim 등)에서
 * 추가된 클레임을 조용히 지워버린다.
 *
