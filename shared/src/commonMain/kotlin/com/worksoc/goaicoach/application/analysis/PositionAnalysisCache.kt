...
const val JsonPositionAnalysisCacheTtlMillis: Long = 365L * 24L * 60L * 60L * 1_000L
internal const val JsonPositionAnalysisCacheMinReusableFillRatio: Double = 0.5

enum class PositionAnalysisCacheOrigin(
    val label: String,
    val trustRank: Int,
) {
...
enum class PositionAnalysisCacheQualityTier(
    val label: String,
) {
    Complete("complete"),
...
data class PositionAnalysisCacheQuality(
...
interface PositionAnalysisCacheStore {
    fun get(
...
interface TrustedPositionAnalysisCacheProvider {
    fun get(
        key: PositionAnalysisCacheKey,
        nowMillis: Long,
    ): PositionAnalysisCacheEntry?
...
object NoopPositionAnalysisCacheStore : PositionAnalysisCacheStore {
    override fun get(
        key: PositionAnalysisCacheKey,
        nowMillis: Long,
...
fun AnalysisResult.cacheQualityFor(limit: AnalysisLimit): PositionAnalysisCacheQuality =
    PositionAnalysisCacheQuality.from(
        rootVisits = rootVisits,
        requestedRootVisits = limit.visits,
    )

fun AnalysisResult.isStorablePositionAnalysisFor(limit: AnalysisLimit): Boolean =
    candidates.isNotEmpty() && cacheQualityFor(limit).isStorable

fun shouldReplacePositionAnalysisCacheEntry(
...
fun bestPositionAnalysisCacheEntry(
...
fun AnalysisResult.withCacheHitSummary(entry: PositionAnalysisCacheEntry): AnalysisResult =
    copy(
        status = status.copy(
