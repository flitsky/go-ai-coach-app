...
### 2026-09-23 정정 — 게이팅 대상은 셋이 아니라 다섯이다 (코드 실측)

⚠️ 위 2장 서두("형세 보기/추천 수/착수 평가가 프리미엄 게이팅 대상")는 낡았다. `FeatureAccessPolicy.unlockOptionsFor()`를
직접 세어 보면 같은 분기(`{AdGrant, Purchase}`)로 묶인 것은 **다섯**이다 — 무르기(Undo)·형세 보기(Eval)·추천
수(Top Moves)·착수 평가(Move Review, 코드상 표시명은 "기보 리뷰")·**바둑판 사진 분석(Board Scan)**. 무르기가
빠져 보였던 이유는 바로 위 2026-08-13 정정으로 이미 다시 게이팅 대상에 들어갔기 때문이고(2장 서두 문장이
갱신되지 않았을 뿐), 바둑판 사진 분석은 **2026-09-21 #179**로 새로 추가된 기능(카메라로 실제 바둑판을
촬영해 국면을 인식 — 실험적 기능 설정 뒤에 숨어 있음)이라 이 문서가 아직 반영한 적이 없다. 정본 개수와
근거는 `FEATURE_ACCESS_PRINCIPLES.md` §7의 2026-09-23 갱신 절 참고.
...
| 단계 | 작업 내용 | 상태 |
| --- | --- | --- |
| Step 1 | 프리미엄 상태값 관리 인프라 | ✅ 완료 (2026-07-28) — `application/premium/state/PremiumState.kt` + 단위 테스트 |
...
- **신규 파일**: `application/premium/port/AdRewardPort.kt`, `application/premium/app/PremiumAdGrantApplication.kt`, `ui/AndroidRewardedAdClient.kt`(같은 날 개명, 이후 `platform/AndroidRewardedInterstitialAdClient.kt`가 맡는다), `shared/.../application/PremiumAdGrantApplicationTest.kt`.
...
- **신규 파일**: `application/premium/port/PurchasePort.kt`, `application/premium/app/PremiumPurchaseApplication.kt`, `platform/AndroidBillingClient.kt`, `PremiumPurchaseGlue.kt`, `shared/.../application/PremiumPurchaseApplicationTest.kt`.
...
### 배너 광고 재노출 위치 — ✅ **집행됨 (2026-09-19, 백로그 #156)**

- 2026-08-08에 홈 화면 하단 배너 광고 호출부를 임시 제거한 뒤(`BannerAdView.kt`/`AdUnitIds.kt` 등 인프라는 그대로 유지, 커밋 `59d880c`), 재노출 위치를 이 세션에서 사용자에게 확인했다.
- **결정(2026-08-09)**: 출시 시점엔 배너 광고를 노출하지 않는다. 향후 "대국 복기하기"(기보 복기) 기능이 추가되는 시점에 그 화면에 배너를 넣을 예정 — 지금 다시 판단할 필요 없이 그 기능 착수 시 함께 결정한다.
- ✅ **2026-09-19에 그대로 집행했다.** 다시보기 화면(#156)이 생기면서 예약해 둔 자리에 붙였다 —
  **헤더 바로 아래, 화면 콘텐츠의 맨 위.** 그 위(헤더 위)로 올리지 않은 이유는 나가는 길인
  뒤로가기가 광고 아래로 밀리기 때문이다(AdMob 「실수 클릭 유도 배치 금지」 취지).
- ⚠️ **구독자에게는 뜨지 않는다.** 판정은 화면이 아니라 `SubscriptionAwareBannerAd`가 갖는다 —
  화면마다 `if (!premium…)`를 적으면 다음에 배너를 붙이는 화면이 그 줄을 빠뜨려도 **컴파일도 되고
  테스트도 초록이다.** 축은 `isPurchased`(구독)이지 `isActive`가 **아니다** — `isActive`는 광고를
  봐서 얻은 1시간을 포함해서, 그걸 쓰면 *"광고를 한 번 보면 한 시간 동안 배너가 사라지는"* 앱이 된다.
  근거는 등재문 [요금 안내]의 *"광고 없이 편하게 쓰고 싶다면 프리미엄 구독"* 이다.
- ⚠️ **크기를 `getLargeAnchoredAdaptiveBannerAdSize`에서 `getPortraitAnchoredAdaptiveBannerAdSize`로
  바꿨다**(2026-09-19 실기). 전자는 411dp 폭에서 **약 129dp**를 예약하는데, 소재가 그보다 작게
  내려오면(테스트 광고는 320x50) 남는 위아래가 **검은 띠**로 남았고 판이 그만큼 줄었다.
  ⚠️ `getCurrentOrientationAnchoredAdaptiveBannerAdSize`는 **쓰지 않는다**(deprecated, 2026-08-05 기록).
  이 앱은 `screenOrientation="portrait"`이라 세로용을 직접 고르는 것이 정확하다.
- ⚠️ **방침·콘솔 답이 함께 움직인다**(함정 60) — `PLAY_CONSOLE_PRIVACY_ANSWERS.md`의 "배너 미배선"
  서술을 같은 날 정정했다. IARC는 **다시 열 필요가 없다**(「광고 포함」이 이미 **예**이고, 새 설문지는
  이전 답을 하나도 안 불러와 177개국 등급이 움직인다 — 함정 58).
...
- **결정 내용**: 프리미엄 자체를 **월 구독으로 전환**한다. `premium_lifetime`(비소모성 영구, ₩9,900)은 이 방향과 맞지 않아
  **2026-09-18에 구매 옵션을 비활성화했다.** ⚠️ 제품은 지우지 않았다 — 상품 ID는 재사용이 불가능하고 기존 소유자 권한을 지켜야 한다.
...
- **가격 확정(2026-08-30)**: **월 3,900원**. 방침은 *"기능이 늘면서 가치가 오르면 그때 가격도 조정"*.
  ...
  · ⚠️ **상품 ID·요금제 이름·출시 국가를 여기 옮겨 적지 않는다.** 콘솔 실측의 정본은 **활성 백로그 #150** 「이미 끝낸 것」이다. 두 곳에 적으면 또 갈린다 — 위가 그 사고 기록이다.

#### ✅ 2026-09-18 콘솔 실측 — "선행 확인" 두 질문이 풀렸다

옛 「선행 확인」 불릿은 *"`premium_lifetime` 기구매 계정을 어떻게 처리할지, 구독 상품을 새로 등록해야 하는지"* 를 물었다. 답은 이렇다.

- **구독 상품은 새로 등록해야 했고, 등록했다.** 일회성 제품은 구독으로 전환되지 않는다. 상세는 백로그 #150.
- **기구매 계정은 건드리지 않는다.** 제품을 지우지 않았으므로 기존 소유자의 권한은 그대로 남는다. 라이선스 테스터도 마찬가지다.
- ⏸️ **기본 요금제는 일부러 활성화하지 않았다.** 지금 켜면 스토어에 결제 배지가 살아나는데 **앱에는 구매 경로가 없고**
  등재문은 *"앱 내 결제는 없습니다"* 라고 말한다 — 비활성화로 막 해소한 모순이 그대로 되살아난다.
...
- ⚠️ **되살리려면 이 사유부터 뒤집을 것.** 자체 만료 시계를 두는 순간 **Play와 앱이라는 두 개의 진실**이
  ...
  - ✅ **2026-09-18 — 뒤집지 않고 비켜 갔다**(백로그 #174, 아래 절). 앱이 둔 것은 **만료 시계가 아니라
    신뢰 한도 시계**라 주장이 겹치지 않는다. 위 경계는 **여전히 유효하다** — 만료일을 앱이 계산하려는
    시도가 다시 나오면 이 줄을 먼저 읽을 것.

#### ✅ 2026-09-18 — **구독 확인 신선도 시계**(24시간 재조회 · 72시간 강등)

**왜 필요했나**(백로그 #173 실기): **강등 계기가 「앱 완전 재시작」 하나뿐이었다.** 앱을 켠 채 만료를
넘겨도, 홈으로 나갔다 돌아와도 권한이 살아 있었다 — 복원 조회가 `LaunchedEffect(Unit)`이라 컴포지션당
한 번이었기 때문이다. **해지한 사용자가 앱을 계속 켜 두면 권한이 계속 살았다.**

**무엇을 뒀나**(사용자 설계, 커밋 `e6454eeb`): `PremiumState.lastSubscriptionVerifiedAtMillis` —
**마지막으로 확인에 성공한 시각.** `SubscriptionFreshnessPolicy`가 그것을 읽어 세 갈래로 답한다.

| 경과 | 판정 | 하는 일 |
| --- | --- | --- |
| < 24시간 | `Fresh` | 아무것도 안 한다 |
| ≥ 24시간 | `NeedsRecheck` | 다시 묻는다. **권한은 그대로** |
| ≥ 72시간 | `Expired` | 안내 팝업 + **즉시** 일반 사용자로 강등 |

⭐ **이것이 「두 개의 진실」을 만들지 않는 이유**: 앱의 시계는 **만료일을 주장하지 않는다.** 정하는 것은
*"마지막으로 확인한 「소유」 답을 언제까지 믿을 것인가"* 하나뿐이고, **Play가 답하는 순간 언제나 Play가
이긴다**(조회로 아무것도 못 정했을 때에만 시계를 본다). 그래서 위 경계를 뒤집지 않고 목적만 이뤘다.

- ⚠️ **#158의 관문은 그대로다 — 이것은 두 번째 경로다.** 즉시 강등은 여전히 `isAuthoritativeNotOwned`
  하나로만 일어난다. 이쪽은 *"아무 답도 못 얻은 채 사흘이 지났다"* 는 **다른 사유**로 내린다.
  ⚠️ 둘을 합치지 말 것 — 합치는 순간 **조회 실패 한 번이 유료 구독자를 내리는** #158의 결함이 되살아난다.
- ⚠️ **`null`은 "확인 실패"가 아니라 "아직 한 번도 확인한 적 없음"이다.** #174 이전 저장값에는 이 키가
  없다. 정책이 그 경우를 **강등이 아니라 재조회**로 받지 않으면, 업데이트를 받은 **유료 구독자가 앱을
  켜자마자 전부 강등된다.**
- **재조회 계기**는 `AppForegroundEvents` — 콜드 스타트와 포그라운드 복귀를 한 훅이 덮는다.
  ⚠️ 앱을 **한 번도 배경으로 보내지 않고** 계속 켜 두는 경우는 아직 안 덮는다(주기 재평가가 필요하다).

##### ⛔ 아직 실측되지 않은 전제 — **오프라인에서 조회가 성공하는가**

위 절의 *"`queryPurchasesAsync`가 오프라인에서도 로컬 캐시로 답한다"* 는 **우리 문서의 주장일 뿐
실측 기록이 없다.** 그것이 참이면 비행기 모드에서도 확인이 "성공"으로 도장 찍혀 **72시간이 영영 안 온다.**

- **판정법**: 함정 56대로 내부 테스트 번들을 올려 **Play로 설치**하고, 구독 중인 라이선스 테스터 기기에서
  비행기 모드로 재시작해 `PurchaseOutcome`을 찍어 본다. 사이드로드로는 안 된다.
  ⚠️ 에뮬레이터로는 불가능하다 — 구글 계정이 없으면 결제 연결 자체가 `BILLING_UNAVAILABLE`로 끝난다(2026-09-18 실측).
- **참이면 처방**: 확인 "성공"의 조건에 **그 순간 실제 인터넷 연결이 있었는지**(`ConnectivityManager`)를
  더한다. 그러면 캐시된 답이 시계를 되돌리지 못한다.

##### ⚠️ 여기서 멈춘다 — 위변조 단속으로 넘어가지 말 것

이 시계는 **기기 시각을 그대로 믿는다.** 기기 시각을 마지막 확인 시각 근처에 멈춰 두면 72시간은
영원히 오지 않는다 — **막을 방법이 없고, 막지 않기로 했다**(2026-09-18 사용자 철학).

- ⚠️ 코드의 *"확인 시각이 미래면 낡은 것으로 본다"* 가드는 **방어선이 아니라 데이터 온전성 검사**다.
  그것을 방어선으로 오해하고 위에 무언가를 더 쌓지 말 것.
- **정본은 `FEATURE_ACCESS_PRINCIPLES.md` 8.8** — *"단속하지 않는다, 온라인의 값어치를 키워서 이기는 쪽"*.
  상시 온라인 요구·시계 무결성 검사·기기 위변조 단속을 **넣자는 제안이 나오면 그 절을 먼저 읽을 것.**
- ⚠️ 다만 **위 실측 과제는 그 철학과 무관하다** — 그것은 단속이 아니라 **72시간 규칙이 정직한 경우에
  실제로 작동하는지**의 문제다. 둘을 섞지 말 것.
...
- 옛 「캐릭터 보유와의 관계」(매판 N회 우선 소진)와 「관련: 개별 구매 ₩4,900」 두 불릿은 **무효다.**
  팔지 않으므로 특전도 가격도 없다. **매판 N회는 구현된 적이 없다.**
- **구독이 파는 것 셋**(정본은 `FEATURE_ACCESS_PRINCIPLES.md` **8.1**): ⓐ 모든 캐릭터 ⓑ 대국 중 무제한 ⓒ 대국 종료 후 기능(미구현).
  ⓐ는 백로그 **#157**이 구현했다.
- ✅ **2026-09-23 기준 완료.** #157(로스터 개방) → #158(구독 강등 코어) → #159(고지 UI·오퍼 규칙) →
  #162(결제 플래그 켜기) 전부 완료 커밋이 있다 — 각 번호로 `git log --grep "backlog #NNN"`으로 확인할 것.
  이력은 여기 다시 옮겨 적지 않는다(정본은 git). 남은 것은 이 트랙의 일이 아니라 **출시 절차**(내부
  테스트 트랙 승격·프로덕션 제출)뿐이며, 그 상태는 `GOOGLE_PLAY_LAUNCH_PLAN.md`가 정본이다.
- ⚠️ **결제 코드는 지우지 않는다** — `AndroidBillingClient`를 구독과 공유한다.
- ⚠️ **파는 것은 월 구독 3,900원 하나다.** 캐릭터 개별 판매·`premium_lifetime`(₩9,900) 옛 가격표는
  2026-09-18 사용자 결정으로 폐기됐다(위 절 참고) — 이 문서 위쪽의 그 가격/개별판매 서술은 **당시
  시점의 이력**이지 현재 판매 중인 상품이 아니다.
