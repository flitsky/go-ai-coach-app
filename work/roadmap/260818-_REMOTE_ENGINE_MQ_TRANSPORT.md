...
> 2026-09-23에 `REMOTE_ENGINE_MQ_TRANSPORT_KICKOFF_PLAN_260818_0825.md`에서 지금 이름으로 **개명**했다(옛 이름의 파일은 없는 것이 정상이다). 사유: `work/roadmap/README.md`가 *"가장 중요한 성질"* 이라 부르는 **파일명 정렬 = 시간순** 이 이 문서 하나 때문에 깨져 있었다 — 날짜 접두사가 없어 ASCII 정렬상 숫자로 시작하는 형제들 뒤로 밀렸다. 이 폴더에 남는 것은 맞다(앱 이식 승인 대기 = 완결일 없음).
