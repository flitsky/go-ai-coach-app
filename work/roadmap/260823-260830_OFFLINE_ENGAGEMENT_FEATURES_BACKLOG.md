...
> #40~#42 — 마지막이 #40, 스토어 스크린샷 재캡처와 비공개 테스트 AAB 빌드다.
> ⚠️ 그때 만든 804는 **콘솔에 올라가지 않았다** — 같은 날 저녁 #43~#46이 들어가며 805로
> 대체됐다. 현재 업로드 대상은 `GOOGLE_PLAY_LAUNCH_PLAN.md` §0에 있다.
>
> **여기 없는 것 둘**:
> - **#18(봇 캐릭터 개별 구매)** — 구현은 끝났고 Play Console 수익 창출 게이트에만 막혀 있다.
>   이번 릴리즈에는 필요 없다(플래그 `false` + 유료 캐릭터 0종 = 배선이 잠들어 있다).
>   `260830-260831_POST_LAUNCH_ENHANCEMENTS.md`로 **이관**했다.
...
> 그대로 넣으면 스레드가 할 일을 못 찾는다. 같은 형식의 최신 프롬프트와 절차 전문은
> **`260911-260916_GAMEPLAY_UX_AND_POLICY_COMPLIANCE.md`** 에 있다(2026-08-31에 세대가 한 번 더 넘어갔다 — 중간 세대인
> `260830-260831_POST_LAUNCH_ENHANCEMENTS.md`도 봉인됐다). 아래는 **당시 원문 보존**이다.
...
(파일명만 있어도 검색해서 찾을 수 있지만, 위처럼 `docs/work/roadmap/` 경로를 포함해두면 탐색 한 단계를 줄일 수 있습니다.)
...
- ⚠️ **프리미엄 월 구독 전환은 이 백로그 밖이다**(2026-08-24 결정). 지금의 `premium_lifetime`(비소모성 영구)은 그 방향과 맞지 않지만, 상품 타입·만료/갱신 축·복원 로직이 전부 바뀌는 작업이라 `PREMIUM_MODE.md` 트랙에서 별도로 다룬다. 내부 테스트 트랙에만 게시된 상태라 실구매자 마이그레이션 부담은 없다.
...
23. 5단계 관장 천원을 28일차 출석 장기 보상으로 전환 (AI 모델: Opus, 노력정도: 낮음) [완료]
    - 참고: 킥오프 플랜 7장, `FEATURE_ACCESS_PRINCIPLES.md` 8.3-1절. **#18이 콘솔 게이트에 막혀 발행된 항목이다.**
    - **배경**: Play Console "수익 창출"이 *"대시보드에서 앱 설정을 완료하세요"* 로 잠겨 4,900원 상품 등록이 불가능했다. 비공개 테스트 때문이 아니라 **앱 설정 대시보드 미완료 항목(개인정보처리방침 필드·IARC·데이터 보안)이 선행 조건**인 것으로 보이며, 판매자 설정이 추가로 필요할 가능성도 남아 있다. 그 셋은 `GOOGLE_PLAY_LAUNCH_PLAN.md` §0의 남은 콘솔 작업과 같은 목록이다.
    ...
    - 산출물: `application/botcharacter/BotCharacterCatalog.kt`(`Attendance(28)` + `TopCharacterAttendanceTier`), `application/attendance/AttendanceRewardPolicy.kt`(캐릭터만 실제 회차 조회), `FEATURE_ACCESS_PRINCIPLES.md` 8.3-1절, 테스트 갱신 5건.
    - 사용자 승인 완료(2026-08-29).
...
40. 스토어 스크린샷 재캡처 + 비공개 테스트 AAB 빌드 (AI 모델: Opus, 노력정도: 중간) [완료]
    - **스크린샷 5장 전부 재캡처**(HEAD `5b294f5`, v0.8.3 빌드 `v260830.1635`). 오전에 찍은 세트가
      ...
      바꿨다. 컷 안에서 달라진 것은 2026-08-30 라운드 폴더의 `README.md`에 적었다.
    ...
    - **AAB 빌드 완료**: `make play-internal-aab TARGET=emu` → `dist/go-ai-coach-play-internal.aab`
      (111MB, 당시 `VERSION_CODE=804` / `VERSION_NAME=0.8.4`, release keystore 서명).
      매니페스트에서 `versionName`과 `VIBRATE`를 직접 확인했다.
      ⚠️ **804는 콘솔에 올라가지 않았다** — 같은 날 저녁 #43~#46이 들어가면서 805로 대체됐다.
      현재 업로드 대상과 해시는 `GOOGLE_PLAY_LAUNCH_PLAN.md` §0을 볼 것.
...
18. 봇 캐릭터 개별 구매 배선 — **이관됨(2026-08-30).** 현재 위치는 `260911-260916_GAMEPLAY_UX_AND_POLICY_COMPLIANCE.md` [이관]
...
여기에 덧붙이지 말 것 — 새 일감은 **`260911-260916_GAMEPLAY_UX_AND_POLICY_COMPLIANCE.md`** 가 받는다
(2026-08-30에는 `260830-260831_POST_LAUNCH_ENHANCEMENTS.md`가 받았으나 2026-08-31에 봉인됐다).
