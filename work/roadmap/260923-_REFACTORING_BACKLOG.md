...
세대 시작: 2026-09-23 · 기준선: 1.0.0(versionCode 10000) · `make test TARGET=emu` 초록 · 재정비: 2026-09-26(실기 일감을 번호별 개별 세션으로)
...
📑 **일감 섹션은 넷이고 이 순서를 유지하며 갱신한다**: 「완료 항목」 → 「진행 중」 → 「예정사항」 → 「사용자 결정 대기」.
예정사항의 카드마다 **선행 · 동시 금지 · 🧪 실기 확인**을 적는다 — 번호 하나만 보고 다른 세션이 바로 집을 수 있어야 한다.

---

## 신규 스레드 착수 프로토콜

**고정 프롬프트 ① — 번호를 지정해 한 일감만**(실기 확인 일감은 이걸로 연다. `#N`만 바꿔 복사):

> 이번 스레드에서는 `work/roadmap/260923-_REFACTORING_BACKLOG.md`의 **#N**을 맡아 주세요. 카드의 「선행」과
> 「동시 금지」를 먼저 확인하고, 걸리는 것이 없으면 「진행 중」으로 옮긴 뒤 착수하세요. 구현과 게이트가 끝나면
> 기기에 설치하고 카드의 🧪 시나리오를 제가 따라 할 수 있게 단계별로 알려 주세요. 제가 실기 확인을 마쳤다고 하면
> main에 합치고 문서를 완료로 갱신한 뒤 끝났다고 알려 주세요.

**고정 프롬프트 ② — 다음 차례를 알아서**:

> 이번 스레드에서는 `work/roadmap/260923-_REFACTORING_BACKLOG.md`를 읽고 **「진행 중」의 첫 번째 항목**을
> (비어 있으면 **「예정사항」에서 선행이 끝났고 동시 금지에 걸리지 않는 첫 번째 항목**을) 맡아 주세요. 항목을 이해했으면
> **「진행 중」으로 옮기고** 착수하시고, 결과물을 저와 논의한 뒤 제가 완료를 승인하면 문서를 완료로 갱신하고 끝났다고 알려 주세요.

**착수 직후 할 것 넷** — 순서가 있다.

1. **아래 「⚠️ 반드시 알아야 할 함정」을 읽는다.** 걸리는 키워드가 있으면 `docs/spec/PITFALLS.md`의
   해당 번호 전문을 편다. **84건을 다 읽지 않는다.**
2. **진단서에서 그 항목이 속한 절을 읽는다.** 왜 이 일을 하는지, 무엇을 건드리면 안 되는지가 거기 있다.
3. **선행 항목이 완료인지, 동시 금지 항목이 「진행 중」에 없는지 확인한다.** 카드의 두 줄과 아래 「동시 진행 금지 조합」 참고.
   걸리면 **집지 않는다.**
...
**실기 확인 일감의 마감 — 순서를 바꾸지 않는다**

1. 게이트 초록(위 4번).
2. 기기 설치: `make install-dev-engine TARGET=emu`(실제 KataGo가 필요한 일감은 `TARGET=phone`).
   저장 형식에 닿는 일감(`#22`·`#67`)은 **수정 전 빌드로 저장을 만든 뒤, 앱을 지우지 않고 새 빌드를 덮어 설치**한다 —
   `make reinstall-dev-engine`은 앱을 지우므로 이 확인에 쓰지 않는다.
3. 카드의 🧪 시나리오를 사용자에게 단계별로 건넨다 — 무엇을 누르고, 무엇이 보이면 통과인지. 결함을 고치는 일감은
   **구현 전에 수정 전 빌드에서 재현**해 보이고 같은 시나리오를 새 빌드로 다시 넘긴다.
4. 🔴 **사용자가 실기 확인을 마쳤다고 말하기 전에는 main에 합치지 않는다** — 릴리스(`make bundle-aab`)는 main에서 나간다.
5. 확인이 끝나면 최신 main 위로 다시 올리고(`git rebase` 또는 `git cherry-pick -x`) 게이트를 다시 돌린 뒤 푸시하고,
   카드를 「완료 항목」 표 한 줄로 옮긴다(실기 확인 날짜를 그 줄에 적는다).

**caveat 셋**
- **AI 모델 표기는 사람이 스레드를 띄울 때 고르라는 것**이지 스레드가 스스로 바꾸는 값이 아니다.
- **`docs/DOCS_INDEX.md`를 항목마다 건드리지 않는다.** 문서 등재는 트랙이 끝날 때 한 번에 한다.
- **세션을 여럿 띄워도 된다 — 단 「동시 진행 금지 조합」을 지킨다.** 공유 main 트리에서는 `git checkout`·`reset --hard`·`stash`를
  쓰지 않고(함정 72), 커밋은 경로를 지정한다(함정 80·83). 대량 이동은 격리 워크트리에서 한다.
...
- **함정 82 — 파일을 옮기면 `lint-baseline.xml`의 경로가 낡는다.** 기준선에 묻어 둔 경고가 새 Warning으로 되살아나는데
  게이트는 초록이다(함정 79). 옮기는 커밋에서 baseline 경로를 함께 고친다.
- **함정 83 — `git mv` 뒤 경로 지정 커밋에는 옛 경로도 넣는다.** 새 경로만 넣으면 옛 경로의 삭제가 빠져 커밋 트리에
  파일이 두 벌 남는다(`#70`에서 실제 발생).
- **함정 84 — `spotlessApply`는 격리 워크트리에서만.** 공유 트리에서 돌리면 남의 미커밋 파일까지 재배열한다.
...
계층을 **패키지 FQN + `internal` → (최후) Gradle 모듈**로 물리화한다(가운데 단이던 Konsist는 실측으로 기각 — 「서 있는 답」).
Hilt(commonMain 불가)·Koin(이득 0)·전면 MVI(이미 절반 작동)·모듈 우선 개편(SCC를 모듈이 원리적으로 못 잡음)은
근거와 함께 **기각**했다. 재론 시 진단서 §3.1이 답이다.

### 지금 어디까지 왔나 (2026-09-26)

- ✅ **실기가 필요 없는 구조 정리는 거의 끝났다** — 가드 복구·게이트(P0·P1), 이름공간 정렬(P3 중 `#27`·`#31`·`#67` 말고 전부),
  사이클 절단(`#32` — `commonMain` 패키지 SCC **0**, 바이트코드로도 0)과 계약 좁히기(P4 전부). 「완료 항목」 표 참고.
- 🟡 **남은 실기 불필요 일감**: `#46`(예산 지표, 조율 스레드 진행 중) → 이어서 `#48`·`#27`. `#108`(진단 로그의 엔진 이름 동결, `#43`이 찾음)도 실기 불필요. `#43`·`#106`은 끝났다.
- 🧪 **나머지는 전부 실기 확인이 필요하다** — 「예정사항」 A~D. 사용자가 번호를 골라 개별 세션으로 연다(고정 프롬프트 ①).
- ⏸️ **사용자 결정이 먼저인 것**: P6(`#49`~`#57`) 착수 여부 — `#43`·`#46`·`#48`·`#27`이 정리된 뒤 정한다(2026-09-26). 「사용자 결정 대기」.

### 의존 그래프 — 남은 일감

```
[#94] ··· 동시 금지 ··· [#22]          [#67] 독립(#106 끝남)
[#107] ··· P5·#48·#27과 동시 금지(GoCoachApp)
[#74] 시간 초과 UX 결정됨(2026-09-26) — 선행 없음
[#14] ──→ [#15] ──→ [#17]                          P2 · #98은 이 줄과 동시 금지
[#46] ──→ [#108] ──→ #40 → #41 → #42 → #44 → #45 → #47  P5 · 직렬 · ui/ 점유 · #46·#108은 실기 불필요
[#48] ──→ [#27] ──→ [#31]                          #43·#46 뒤 착수 · ui/ 단독 점유 · P5와 동시 금지
[#81] ──→ [#49] ──→ P6 나머지                       개별 승인
```

✅ **P6(모듈 승격)의 첫 전제였던 SCC 0은 `#32`로 충족됐다.** 남은 전제는 `#27`(분할 뒤 `ui` 참조 그래프)과
`#81`(착수 직전 소스 ∪ 바이트코드 그래프 재측정)이다.

### 동시 진행 금지 조합 — 세션을 여럿 띄울 때

| 묶음 | 이유 |
| --- | --- |
| `#94` · `#22` | 둘 다 새 대국 설정·저장 경로(덤·판 크기)를 고친다 |
| `#14` → `#15` → `#17` · `#98` | 같은 엔진 파일군(`KataGoProcessEngineAdapter`·`LocalEngineSessionClient`). P2는 이 순서로 직렬, `#98`은 P2와 겹치지 않게 |
| `#74` · `#17` | `#17`이 만드는 공통 실패 타입(`EngineOperationFailure`)을 `#74`가 쓸 수 있다 — 먼저 끝난 쪽에 맞춘다 |
| P5 전부(`#40`~`#47`·`#108`) · `#48` · `#27` · `#74` · `#107` | 전부 `ui/`·`GoCoachApp.kt`를 만진다(`#74`는 시간 초과 팝업). P5 안에서도 직렬 |
| 모든 일감 | `ContractSymbols.kt`·`LayeringContractTest.kt`·이 문서는 자주 겹친다 — main에 합치기 직전에 최신 main 위로 다시 올리고 게이트를 다시 돌린다 |

---

## 완료 항목

끝난 순서대로 적는다(번호 순이 아니다). 한 줄 + 커밋 해시 — 자세한 내용은 `git log --grep "refactor backlog #N"`.
...
| 104 | **무르기 직후 후속 분석 요청이 버려질 수 있다 → 사용자에게 보이는 결함 아님(실기, 2026-09-26)** — 무르기 뒤 재동기화 동안 형세·추천수·기권·통과 버튼이 함께 잠깐 꺼졌다 켜지고(엔진 busy 게이트 — 의도된 동작), 엔진이 한가해지면 다른 경로가 분석을 다시 요청해 버려진 요청이 가려진다. ✅ **사용자 결정(2026-09-26): 고치지 않고 주석만** — 다듬고 싶어지면 `EngineOperationKind.isBlocking` 위 주석에서 시작한다(버튼 조건 두 곳이 그리로 가리킨다) | `ec1eef44` |
...
| 106 | **접바둑 보정 규칙의 단일 출처** — `Ruleset.handicapBonusRule`(N / N−1 / 0)을 앱 두 계가기·판정 화면 집계가 줄·엔진이 함께 읽는다. 엔진은 이름 룰 기본값과 다를 때만 덮어쓴다(GTP `kata-set-rule whiteHandicapBonus`, 분석 쿼리 최상위 `whiteHandicapBonus`) — 지금 룰셋은 계가·명령·쿼리 전부 바이트 동일. 검수가 KataGo v1.16.4 소스로 대조하다 **공식 문서 표의 japanese `friendlyPassOk`가 소스와 다름**을 찾아, 룰 필드를 손으로 옮기는 1차 설계를 걷어냈다. 사보타주(중국 → N−1)에 shared 9·engine 4 빨강 | `691d0280` |
| 43 | **컨트롤러 배선 테스트** — 12개를 실제 `wireGoCoachControllers`로 만들고 ⓐ 배선 중 게터를 읽거나 상태를 쓰면 빨강 ⓑ 컨트롤러마다 *부르기 → 옛 결과 → 값 바꾸기 → 다시 부르기* 탐침(첫 사용 포획까지) ⓒ 협력자 공유(설정 변경이 **같은** `UndoController`의 재동기화를 취소하는지 등) ⓓ 함정 67의 진짜 자리 — `GoCoachApp`의 `wiringContext` remember 키 7개를 소스 계약으로 **무조건** 고정(파서가 무엇을 보든 꺼지지 않는다). 검수 3라운드 — 1차는 사보타주 18개를 놓쳤고, 2차는 키 검사가 스스로 꺼지는 구멍이 있었다. 알려진 동결 둘은 특성 테스트로 고정(→ `#108`). 프로덕션 0줄, @Test +44 | `ec00a9d8` |

---

## 진행 중

_(없음 — 실기 일감은 사용자가 번호를 골라 개별 세션으로 연다. 고정 프롬프트 ①)_

---

## 예정사항

> 🧪 **카드마다 「선행 · 동시 금지」와 실기 시나리오가 있다.** 🧪 줄은 **사용자가 기기에서 할 것과 통과 기준**이다.
> 🟢 표시가 붙은 것(`#46`·`#108`·`#48`·`#27`·`#31`·`#81`)만 실기가 필요 없다 — 이 중 `#46`·`#48`·`#27`은 조율 스레드가 한다.
> 위에서 아래로 권장 순서다. 번호를 골라 고정 프롬프트 ①로 연다.

### A. 사용자에게 보이는 잠재 결함 — 먼저 권장
...
94. **덤의 출처가 둘로 갈려 있다 — 끝난 판·이어하기에서 로비로 가면 섞인다** (AI 모델: Opus, 노력정도: 중간) — `#93` 검수가 찾음, **전부터 있던 구조**
    ...
    · 🧪 **실기**: ① 설정의 대국 조건을 19줄·접바둑 3점(덤 0.5 자동)으로 둔다 ② 판 크기·덤이 다른 판(9줄 맞바둑 덤 6.5 등)을
      이어하기로 열거나 기록에서 분기해 끝까지 둔다 ③ 대국 화면의 「대국 설정」 버튼으로 로비에 간다 → **로비가 설정값(19줄·3점·0.5)을
      보이면 통과**, 앞 판의 판 크기·덤이 섞이면 실패 ④ 그대로 시작 → 시작된 판이 로비에 보인 값과 같으면 통과
      ⑤ 이어하기 판을 몇 수 둔 뒤 설정 화면의 덤이 그 판의 덤으로 바뀌어 있지 않으면 통과. 정확한 재현 경로는 착수 세션이 수정 전 빌드에서 먼저 보인다.
74. 🔴 **5계층이 취소를 다시 삼킨다 — `#16`이 절반만 닫혔다** (AI 모델: Opus, 노력정도: 높음)
    · **선행** 없음 · **동시 금지** `#17`·`#48`·`#27`(팝업·문구가 `ui/`에 든다)
    · ✅ **사용자 결정(2026-09-26)**: AI 착수 탐색이 **시간 초과**되면 사용자에게 **「한 번 더 기다리기」 / 「엔진 다시 시작하기」** 중
      하나를 고르게 한다. 사용자가 **취소**(무르기·나가기)한 경우는 묻지 않고 즉시 멈춘다.
      💡 **이미 같은 두 갈래의 팝업이 있다** — 와치독의 「엔진 응답 지연」(`UiStrings.engineStuckDialog*`: 「엔진 초기화」 · 「닫기(계속 대기)」,
      진단 `engine_turn_watchdog_triggered`). 새 팝업을 따로 만들기 전에 그것과 합칠 수 있는지 먼저 본다 — 합치면 새 문구가 거의 안 든다
      (`UiStrings` 필드 여유가 4개뿐이다, `#48`). 다만 *"닫기(계속 대기)"* 는 엔진이 계속 도는 동안 닫는 뜻이고
      *"한 번 더 기다리기"* 는 **다시 요청**하는 뜻이라 동작이 다르다 — 문구를 바꿀지 착수 세션이 사용자에게 보인다.
    ...
    · 🧪 **실기**(폰 권장 — 실제 KataGo): ① 평소 대국 20수 — AI가 매번 둔다 ② AI가 생각하는 중에 무르기 → 곧바로 멈추고 판이
      무른 상태로 남으면 통과(AI가 뒤늦게 수를 두면 실패) ③ AI가 생각하는 중에 홈으로 나갔다 돌아온다 → 정상 진행
      ④ 시간 초과를 일부러 낸다(방법은 착수 세션이 준다 — 예: 디버그 빌드의 짧은 예산) → 선택 팝업이 뜨고, 「한 번 더 기다리기」는
      같은 판에서 AI가 다시 생각해 두고, 「엔진 다시 시작하기」는 엔진이 다시 뜬 뒤 AI가 둔다. 어느 쪽이든 판·시계·기록이 그대로면 통과
      ⑤ 가장 느린 조건(최고 단계·긴 생각 시간)에서 한 수 대기가 설정 시간을 크게 넘지 않는다(같은 예산으로 두 번 기다리던 것이 사라진다).

107. **무르기 직후 컨트롤러가 다시 만들어져, 대기 중인 재동기화가 버려진 인스턴스에 남는다** (AI 모델: Opus, 노력정도: 중간) — `#43` 검수가 찾고 조사로 확인(2026-09-26)
    · **선행** 없음 · **동시 금지** P5(특히 `#42`)·`#108`·`#48`·`#27`(`GoCoachApp.kt`)
    · 무르기 핸들러가 `sessionSnapshot`·`undoEngineInterventionQuietUntil`을 바꾸면 다음 프레임에 `wiringContext`와 컨트롤러 12개가 **새로 만들어진다**
      (`GoCoachApp`의 remember 키). 대기 중인 재동기화 잡은 `UndoController`의 **인스턴스 필드**(`pendingSync`·`pendingSyncJob`)에 있어 버려진 옛 인스턴스에 남고,
      새 컨트롤러의 `cancelPendingSync()`는 `pendingSync == null`이라 아무것도 안 한다. `6fdbfa70`(2026-07-18, `controllers = remember(wiringContext)`)부터다.
    · 보이는 영향(조사, 코드 기준): ⓐ 무르기 1초 안에 착수하면 `isPendingUndoSync`가 옛 잡이 깰 때까지 true로 남아, 그동안의 **자동 추천 수·착수 평가 요청이 버려지고**
      다시 걸리지 않는다(추천 수·착수 평가를 켠 경우 그 국면만 빠짐). ⓑ 무르기 1초 안의 설정 변경(플레이어 설정·탐색 시간)이 재동기화를 못 취소 — 무해에 가깝다.
      판 손상·설정 덮어쓰기는 **없다**(잡이 판 변경을 보면 스스로 멈추고, 엔진 명령은 매번 판을 다시 맞춘다). 심각도 낮음.
    · 처방 후보: 대기 잡을 컨트롤러 인스턴스 밖(홀더나 한 번만 remember하는 객체)으로 옮기거나, `UndoController`를 재생성에서 빼기. ⚠️ 함정 67 — remember 키를 빼는 방식은 안 된다.
    · 🧪 **실기**: 사람끼리 대국 또는 빠른 초급 AI 대국, **추천 수 켬** → 수를 몇 개 두고 무르기 → **곧바로**(1초 안) 착수 → 새 국면에 추천 수가 뜨면 통과.
      수정 전 빌드에서 빠지는지 먼저 재현한다 — 엔진 타이밍에 따라 재현이 안 될 수도 있고, 그러면 사용자에게 알리고 우선순위를 다시 정한다.
...
22. **`GameSetup` 값 객체 — 덤 유실의 구조적 해법** (AI 모델: Opus, 노력정도: 높음)
    ...
    · 🧪 **실기**: ① **수정 전 빌드**로 13줄·접바둑 2점·덤 0.5·집계가 대국을 몇 수 두고 앱을 강제 종료 ② 새 빌드를 **지우지 않고 덮어 설치**
      → 이어하기에서 판과 네 값(판 크기·룰·접바둑·덤)이 그대로면 통과 ③ 새 빌드에서 같은 조건으로 새 대국 → 강제 종료 → 이어하기에서 그대로
      ④ 대국 기록 목록에서 옛 판이 열린다. ⚠️ 함정 69: 옛 저장이 하나라도 안 읽히면 실패다.
67. **`FinalScoreResult.status: EngineStatus`를 걷어낸다** (AI 모델: Opus, 노력정도: 높음)
    · **선행** 없음(`#106` 끝남) · **동시 금지** 없음
    ...
    · 🧪 **실기**: ① **수정 전 빌드**로 대국 하나를 끝까지 두어 계가 결과를 보고, 다른 대국을 두다 이어하기 상태로 둔다
      ② 새 빌드를 **지우지 않고 덮어 설치** → 이어하기가 열리고, 대국 기록에서 끝난 판의 결과(승자·점수)가 그대로면 통과
      ③ 새 빌드에서 맞바둑·접바둑(면적계가) 대국을 각각 끝까지 → 계가 화면의 승자·점수·요약이 정상이면 통과.
98. **`runStartupBenchmark`의 onProgress 콜백 → `Flow<EngineBenchmarkEvent>`** (AI 모델: Opus, 노력정도: 중간) — **실기(에뮬레이터) 확인**, `#35` 선행
    · **선행** 없음(`#35` 끝남) · **동시 금지** P2(`#14`·`#15`·`#17`)
    ...
    · 🧪 **실기**(에뮬레이터로 충분): 설정 → 개발자 섹션 → 기기 벤치마크 1회 → 진행 팝업이 단계별로 올라가고, 끝에 결과가 **한 번만** 뜨고,
      닫은 뒤 대국 화면에서 AI가 정상으로 두면 통과. 두던 대국이 있었다면 그 판도 그대로여야 한다.

### C. P2 — 엔진 동시성 (직렬: 14 → 15 → 17)
...
14. **프로세스 수명 뮤텍스 + 1계층 실체화** (AI 모델: Opus, 노력정도: 최대)
    ...
    · 🧪 **실기**(폰 권장 — `make install-dev-engine TARGET=phone`): ① 첫 실행 엔진 기동 ② 대국 30수 ③ 무르기 5회 빠르게 연타
      ④ 형세보기·추천 수를 번갈아 5회 ⑤ 홈↔대국 왕복 3회 ⑥ 백그라운드 30초 뒤 복귀 → 전 과정에 멈춤·「엔진 멈춤」 팝업·AI 무응답이 없으면 통과.
      착수 세션은 로그캣의 엔진 진단(재시작 횟수·세대)도 함께 본다.
15. **오퍼레이션 단위 직렬화** (AI 모델: Opus, 노력정도: 최대) — 14 뒤
    ...
    · 🧪 **실기**: `#14`의 ①~⑥ 전부 + ⑦ AI가 생각하는 중에 형세보기·추천 수 → 기다리지 않고 기존 "잠시 뒤" 흐름으로 넘어가고 AI 착수는 정상
      ⑧ 엔진 멈춤 복구(재현법은 착수 세션이 준다)가 대기 없이 된다.
17. **타임아웃 예산 단일화 + 공통 실패 타입** (AI 모델: Opus, 노력정도: 높음)
    ...
    · 🧪 **실기**: ① 단계별(초보·중간·최고)로 AI 한 수 대기 시간이 수정 전과 같거나 짧다 ② 형세보기·추천 수 대기 시간도 같다
      ③ 원격 엔진을 쓰는 디버그 빌드라면, 네트워크를 끊었을 때 한 번 재시도한 뒤 실패를 알리고 앱이 멈추지 않는다.

### D. P5 — 상태 소유자 (가장 위험 · 직렬 · 태스크당 커밋 하나)

> 권장 순서 **46 → 108 → 40 → 41 → 42 → 44 → 45 → 47**(`#43` 끝남). 카드도 이 순서로 놓았다.
> ✅ **사용자 결정(2026-09-26)**: 실기가 필요 없는 `#43`·`#46`을 조율 스레드가 먼저 한다(IDE 업데이트 뒤), 그다음 `#48`·`#27`.
> `#46`을 `#45` 앞으로 당겼다 — 새 지표가 먼저 있어야 `#45`가 전역 `object`를 셸로 되돌릴 때 훅 예산 42/42(여유 0)에 막히지 않는다.
> `#45`는 되돌린 상태의 종류를 화이트리스트에 넣는 것으로 끝난다.

46. **예산 지표 교체** (AI 모델: Opus, 노력정도: 중간) — **45보다 먼저**(2026-09-26 순서 변경)
    · **선행** 없음(`#45`보다 먼저로 바뀜 — 위 순서 메모) · **동시 금지** P5 나머지 · 🟢 **실기 불필요**(가드·설정 파일)
...
108. **진단 출력의 엔진 이름이 처음 값으로 굳는다 — 두 자리** (AI 모델: Opus, 노력정도: 낮음) — `#43`이 찾음
    · **선행** `#46` · **동시 금지** P5 나머지·`#48`·`#27` · 🟢 **실기 불필요**(디버그 리포트·런타임 로그 문구만 바뀐다)
    · ⓐ `wireDebugReportController`(`SettingsAndDiagnosticsControllerWiring.kt`)가 `engineName`·`engineDiagnostic`을 **배선 시점 값으로** 넘긴다 — 지금은 `isEngineReady` 키가
      컨트롤러를 다시 만들어 가려져 있을 뿐, 함정 67 ⓐ~ⓒ를 진행하면 디버그 리포트가 첫 엔진 이름에 얼어붙는다.
      ⓑ `GoCoachApp`의 `val lifecycleController = remember { EngineOperationLifecycleController(currentRuntimeLogContext = { currentRuntimeLogContext() }, …) }`에 **키가 없어**
      첫 컴포지션의 지역 함수(그때의 `engineName`·`engineDiagnostic`, 대개 Unresolved)를 쥔다 — 엔진 수명 런타임 로그가 프로세스 내내 그 값이다.
      ⚠️ ⓑ에 **키를 달면 안 된다** — `lifecycleState`·`activeJobs`를 쥔 컨트롤러가 다시 만들어진다. 지연 읽기(`rememberUpdatedState` 식)로 푼다.
    · `#43`의 특성 테스트 둘(`debugReportEngineNameAndDiagnosticAreTheKnownLatentFreeze`, `theLifecycleControllerRuntimeLogContextIsTheKnownFirstCompositionFreeze`)이
      고치는 순간 빨개지며 무엇을 지울지 안내한다.
    · 문서 한 줄(함께): `docs/spec/PITFALLS.md` #67의 *"위 8개"* 에 `searchTimeSettings`가 들어 있으나 지금 object 멤버는 그것을 읽지 않는다(`#43` 계약의 목록이 정답).
      PITFALLS는 고치지 않는 규칙이라 활성 백로그 색인 67번에 ⚠️로 적는다.
40. **`ViewModel` 얇은 래퍼** (AI 모델: Opus, 노력정도: 최대)
    · **선행** `#108`(`#43` 끝남) · **동시 금지** P5 나머지·`#48`·`#27`
    ...
    · 🧪 **실기**: 위 인수 기준 그대로 — 「액티비티를 유지하지 않음」 ON → 대국 20수 → 홈 → 복귀 시 판·화면 위치가 살아 있으면 통과(수정 전 빌드에서는 실패해야 한다).
41. **`GameSessionScope` 도입** (AI 모델: Opus, 노력정도: 높음)
    · **선행** `#40` · **동시 금지** P5 나머지·`#48`·`#27`
    · 대국 하나가 끝났을 때 무엇이 정리되고 무엇이 남는지가 지금 정의돼 있지 않다(전부 앱 전역).
    · 🧪 **실기**: 대국을 끝내고 곧바로 새 대국 → 앞 판의 분석·형세 그래프·AI 생각이 새 판에 섞이지 않으면 통과. 기권·통과로 끝낸 경우도 1회씩.
42. **`WiringContext` 5분할** (AI 모델: Opus, 노력정도: 최대)
    · **선행** `#41`(`#43` 끝남 — 배선 테스트가 이 분할의 그물이다) · **동시 금지** P5 나머지·`#48`·`#27`
    ...
    · 🧪 **실기**(함정 67의 동결 확인): 대국 중 설정에서 난이도·언어·글꼴 크기를 바꾸고 대국 화면으로 돌아와 **바뀐 값으로 동작**하면 통과
      (AI 세기 표시, 문구 언어, 엔진 이름·상태 표시). 하나라도 옛 값이면 실패 — CI는 이것을 못 잡는다.
44. **화면 상태 3분할 + `UiState` 파일 3개 분리** (AI 모델: Opus, 노력정도: 높음)
    · **선행** `#42` · **동시 금지** P5 나머지·`#48`·`#27`
    ...
    · 🧪 **실기**: 프리미엄·캐릭터 선택·소모품 화면을 각각 열어 수정 전과 같은 모양·동작이면 통과(구매는 누르지 않아도 된다 — 버튼 활성 상태만 본다).
45. **전역 `object` 회수** (AI 모델: Opus, 노력정도: 중간)
    · **선행** `#44`(`#46`은 이미 앞에서 끝난다) · **동시 금지** P5 나머지·`#48`·`#27`
    · `SplashVisibility`·`FinishedGameFlow`·`AppFontScaleState` — 코드에 사유가 **"훅 예산 절약"** 이라고 적혀 있다.
    · 🧪 **실기**: ① 첫 실행 스플래시가 한 번만 ② 대국 종료 → 결과 → 기록 흐름이 전과 같다 ③ 글꼴 크기 변경이 즉시 반영되고 재시작 뒤에도 유지.
47. **`AppContainer` + Compose 성능** (AI 모델: Opus, 노력정도: 높음)
    · **선행** `#45` · **동시 금지** P5 나머지·`#48`·`#27`
    ...
    · 🧪 **실기**: 주요 화면 한 바퀴(홈·로비·대국·기록·설정·공부) — **값이 바뀌면 화면도 바뀌는지**(착수 표시·점수·타이머·토글 스위치).
      `@Immutable` 오용은 "화면이 안 바뀌는" 조용한 버그라 게이트가 못 잡는다.

### E. `ui/` 단독 점유 — `#43`·`#46` 뒤 조율 스레드가 착수

> ✅ **사용자 결정(2026-09-26)**: IDE 업데이트 뒤 `#43`·`#46`을 먼저 끝내고(`#43` 끝남) `#48` → `#27` 순으로 착수한다. 착수 때는 설계의 S0 재실측부터.
>
> 🤖 **모델 배분 — Haiku는 권하지 않고, 커밋의 대부분은 Sonnet으로 충분하다.** 이 둘의 위험은 타이핑이 아니라 **조용한 함정**
> (lint-baseline 경로 — 함정 82, `git mv` 옛 경로 — 83, 공유 트리 거짓 판독 — 77, JVM 슬롯 한도, 가드를 고치고 코드를 두는 실수)이고,
> 그중 기계적 이동은 **컴파일러·골든이 전부 잡는다**. 그래서 판단이 드는 몇 걸음만 Opus, 나머지는 Sonnet, 걸음마다 검수는 Opus로 나눈다.
> 토큰의 대부분은 기계적 걸음에서 나가므로 절감의 대부분이 여기서 나온다(커밋 기준 대략 70~80%가 Sonnet).
>
> | 걸음 | 모델 | 이유 |
> | --- | --- | --- |
> | `#48` C1 골든 스냅샷 · C3~C11 묶음 이동 · C12 · D1 죽은 문구 · F1~F3 포매터 | Sonnet | 골든(4개 언어 × 모든 필드)이 글자 하나까지 증명 |
> | `#48` C2 한도 가드를 `copy$default` 바이트코드로 · S1~S9 위성 Map → 필드 · R1 래칫 | Opus | 가드 설계, `Map.getValue` 실패 모드를 없애는 판단 |
> | `#27` C2~C16 패키지 이동(아래층부터) | Sonnet | package·import만 — 컴파일러가 완전성 보증 |
> | `#27` C0 경로 일원화 · C1 선언 4개 분리 · C1b·C17 래칫 | Opus | 사이클을 끊는 자리 선택, 래칫 기준선 |
> | `#27` C18 문서 경로 74곳 | Sonnet(Haiku도 가능) | `check-doc-links.py`가 기계적으로 검증 |
> | 걸음마다 검수(적대적) | Opus | 함정 82·83·77을 잡는 눈 |

48. **`UiStrings` 분해 — 차단 해제** (AI 모델: Opus, 노력정도: 최대) — **27보다 먼저**
    · **선행** `#43`·`#46`(사용자 결정 순서) · **동시 금지** P5 전부·`#27`·`#74` · 🟢 **실기 불필요**
...
### F. P6 — 선택적 확장 (개별 승인 · 하나도 안 해도 완결)

> ⏸️ **승인 전엔 집지 않는다** — 「사용자 결정 대기」 표에 각각 물을 것이 있다.

81. **사이클 래칫이 타입 추론으로만 생기는 의존을 못 본다** (AI 모델: Opus, 노력정도: 중간) — **최소분 끝, P6 직전에 한 번 더**
    · **선행** 없음 — 단 **`#49` 착수 직전에** 잰다(지금 재면 다시 재야 한다) · 🟢 **실기 불필요**
...
> 결정이 나면 이 표에서 지우고, 결정 내용을 해당 카드에 `✅ 사용자 결정(날짜)` 한 줄로 옮긴다.
>
> ✅ **2026-09-26 결정분(카드로 옮김)**: `#48`·`#27` → `#43`·`#46` 뒤 착수(E 그룹) · `#74` → 시간 초과 시 「한 번 더 기다리기 / 엔진 다시 시작하기」 선택 · `#104` 후속 → 고치지 않고 주석만(완료 표 `#104` 줄) · 아래 P6은 **`#43`·`#46`·`#48`·`#27`이 정리된 뒤** 정한다.

| 항목 | 정해 주실 것 | 정하기 전까지 |
| --- | --- | --- |
| `#49` (모듈 승격) | P3·P4가 끝나 실제 import 그래프가 드러난 뒤 착수 여부를 정한다. **그 전에 목표 그래프를 확정하지 않는다.** 남은 전제는 `#27` | 안 한다 |
| `#50` (build-logic) | 모듈이 늘어나는 `#49`와 묶어 판단 — 모듈 셋에서는 중복이 적어 이득이 작다 | 안 한다 |
| `#51` (디자인 시스템) | 색·치수를 토큰으로 모을까 — 화면 모양이 바뀔 수 있어 실기 확인이 필요하다 | 안 한다 |
| `#52` (바둑판 접근성) | 스크린리더로 대국할 수 있게 할까 — 제품 우선순위 질문 | 안 한다 |
| `#53` (오류 가시성) | 프로덕션 오류를 모을까, 어떤 방식으로 — 수집 방식에 따라 콘솔 개인정보 답안(A-2~A-4)에 닿을 수 있다 | 안 한다 |
| `#54` (MQ 이식) | 제품 기능인가 기술 PoC인가 — 활성 백로그 U-33·34와 같은 질문 | 안 한다 |
| `#55` (툴체인) | AGP 9.0 등이 콘솔·라이브러리 요구로 실제로 필요해졌는가 | 안 한다 |
| `#56` (숨은 난이도) | 숨겨 둔 초급/중급/고급 코드를 대국장 로드맵에서 되살릴까, 지울까 | 코드 보존 |
| `#57` (PAD) | 109MB를 줄일 필요가 실제로 생겼는가(설치 이탈 관측 등) | 안 한다 |
