...
#### 🔍 실기 확인 대기열 — main 빌드 하나로 한 번에 (2026-09-25)

> 기기가 둘 붙어 있으면 `make`가 doctor에서 멈춘다 — `ANDROID_SERIAL=<serial>`을 앞에 붙인다. 이어하기 대국이 남아 있으면 설정 칸이 잠기니 기권하고 홈에서 시작.

**#93 — 접바둑을 고르면 덤 0.5** (로비와 설정 화면에서 각각)
1. 접바둑 없음·덤 6.5 → 접바둑 3점 → 덤 **0.5** · 2. 3점 → 5점 → **0.5 그대로** · 3. 없음으로 → **6.5**
4. 3점(0.5)에서 덤을 7.5로 고친 뒤 없음으로 → **7.5 유지** · 5. 3점으로 두고 앱 재시작 → **3점·0.5 유지**
6. 맞바둑·덤 7.5로 두고 재시작 → 판 크기 바꾸기 → **7.5 유지**(전엔 6.5로 돌아갔다)

**#92 — 백이 첫 수 전에 패스해도 접바둑 보정이 2로 유지**
1. **엔진이 이미 깔린 기기**에 덮어 설치한다 — `make install-dev`만(⚠️ `reinstall-dev-engine`·`seed-engine`은 cfg를 새로 깔아 이 수정의 요점을 가린다).
2. 앱에서 대국을 시작하고 추천수를 한 번 열어 두 엔진을 다 띄운 뒤:
   `adb shell run-as com.zenit9hub.ai.baduk sh -c 'grep -l "Config override: assumeMultipleStartingBlackMovesAreHandicap = false" files/katago/logs/*.log'` → 최신 gtp·analysis 로그 둘 다 나와야 한다.
3. 9x9 · 면적계가 · 접바둑 2점(덤은 0.5로 자동) · 2인 대국 → **백이 첫 수로 패스**, 흑이 한 수 → 이어서 정리될 때까지 두고 패스·패스.
   디버그 리포트의 EndgameLog에서 `diagnosticKataGoFinalScore=` 가 `localAreaAfterCleanup=` 와 같고, `score.final_disagreement` 진단 이벤트가 **없어야** 한다.
4. 회귀: AI와 맞바둑 한 판씩(집계가, 면적계가)을 끝까지 — 수·그래프·결과가 평소대로, 면적계가 판은 위 두 값이 같다.
...
72. **import 정렬이 깨졌다** (AI 모델: Sonnet, 노력정도: 낮음)
    ...
    · 📐 **조사(2026-09-25)**: 어긋난 파일 **238/556**(android_studio 레이아웃 기준). `#24` 전에도 152개였다 — 대량 이동마다 수십 개씩 는다.
      ...
      **`#48`·`#27` 전에** 해야 그 두 대량 이동이 처음부터 정렬돼 들어온다. ✅ **사용자 결정(2026-09-25): 추천대로 진행.**
      착수 조건: 진행 중인 파도(2파도·#84·2b·2c)를 main에 다 합친 뒤, 새 main에서 격리 워크트리로 — C1 정렬(단일 대량 커밋, import 줄만) → C2 게이트
      (버전 카탈로그·모듈 3개 spotless 블록·`.editorconfig`·Makefile `test`에 app/engine `spotlessCheck`·`.git-blame-ignore-revs`). 순서를 거꾸로 하면 그 사이 main이 빨갛다.
...
92. **KataGo가 백의 첫 착점 전 흑돌을 전부 접바둑 돌로 센다** (AI 모델: Sonnet, 노력정도: 낮음) — `#89` 검수가 찾음, 면적계가에서만
    · 앱 cfg(`friend/assets/katago/gtp_learning.cfg`)가 `assumeMultipleStartingBlackMovesAreHandicap`를 안 적어 기본값 true다.
      ...
      **맞바둑에서도** 흑·백 패스·흑이면 KataGo N=2로 앱(0)과 승자가 갈린다. 최종 승패는 앱 계가가 정하므로 결과는 앱 규칙대로지만,
      **AI의 형세 판단·수 선택이 다른 규칙**으로 돈다. KataGo 백은 9x9 5점에서 1수째에 곧바로 패스하기도 한다(실측).
    · 처방: cfg에 `assumeMultipleStartingBlackMovesAreHandicap = false` 한 줄(검수가 세 판 모두 앱 규칙과 같아짐을 확인). 엔진 설정 변경이라 빠른초급 한 판 실기 확인.
    · 덤: `StubEngineAdapter.estimateScore`의 보정 빼기 한 줄은 어떤 테스트도 지키지 않는다(스텁 전용, 제품 영향 0).
93. **접바둑에서도 덤 기본값이 6.5다** (AI 모델: Opus, 노력정도: 낮음) — **제품 판단**, `#89` 조사가 찾음
    · 한국 관습은 접바둑에서 덤 0 또는 0.5다. 지금은 접바둑을 골라도 덤이 6.5에 머물고, 덤 선택지는 0.5/6.5/7.5뿐이다.
      접바둑을 고르면 덤을 0.5로 바꿀지(사용자가 되돌릴 수 있게), 그대로 둘지는 사용자가 정할 일이다.
    · ✅ **사용자 결정(2026-09-24)**: *"핸디캡 대국에서는 0.5점으로 자동 스위칭하기."* 세부(오케스트레이터): 0 → 2점 이상이면 덤 0.5,
      2점 이상끼리 바꾸면 덤 유지, 2점 이상 → 0이면 덤이 0.5 그대로일 때만 기본 6.5로. 전환 뒤에도 덤은 자유롭게 고른다.
      **이관 없음**(이미 저장된 "접바둑 + 덤 6.5"는 그대로), 진행 중·저장 대국은 영향 없음. 워크트리에서 구현·검수 중, 실기 확인 필요.
