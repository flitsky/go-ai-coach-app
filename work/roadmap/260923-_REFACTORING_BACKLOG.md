...
- 🟡 **남은 실기 불필요 일감은 둘**: `#43`(P5의 첫걸음 — 테스트만 추가), `#46`(예산 지표). 조율 스레드가 IDE 업데이트 뒤 이 순서로 하고, 이어서 `#48`·`#27`. `#106`은 끝났다.
...
[#43] ──→ [#46] ──→ #40 → #41 → #42 → #44 → #45 → #47   P5 · 직렬 · ui/ 점유 · #43·#46은 조율 스레드
...
| P5 전부(`#40`~`#47`) · `#48` · `#27` · `#74` | 전부 `ui/`·`GoCoachApp.kt`를 만진다(`#74`는 시간 초과 팝업). P5 안에서도 직렬 |
...
> 🟢 표시가 붙은 것(`#43`·`#46`·`#48`·`#27`·`#31`·`#81`)만 실기가 필요 없다 — 이 중 `#43`·`#46`·`#48`·`#27`은 조율 스레드가 한다.
...
> 권장 순서 **43 → 46 → 40 → 41 → 42 → 44 → 45 → 47**. 번호 순이 아니라 이 순서다.
...
43. **컨트롤러 배선 테스트** (AI 모델: Opus, 노력정도: 높음) — 42보다 **먼저**
    · **선행** 없음 · **동시 금지** P5 나머지 · 🟢 **실기 불필요**(테스트만 추가 — 프로덕션 코드 0줄)
...
> ✅ **사용자 결정(2026-09-26)**: IDE 업데이트 뒤 `#43`·`#46`을 먼저 끝내고 `#48` → `#27` 순으로 착수한다. 착수 때는 설계의 S0 재실측부터.
>
