...
- 🟡 **남은 실기 불필요 일감은 셋**: `#106`(진행 중), `#43`(P5의 첫걸음 — 테스트만 추가), `#46`(`#45` 뒤).
- 🧪 **나머지는 전부 실기 확인이 필요하다** — 「예정사항」 A~D. 사용자가 번호를 골라 개별 세션으로 연다(고정 프롬프트 ①).
- ⏸️ **사용자 결정이 먼저인 것**: `#48`·`#27`(→`#31`) 착수 시점, `#74`의 시간 초과 동작, P6 전부 — 「사용자 결정 대기」.
...
[#43] ──→ #40 → #41 → #42 → #44 → #45 → #46 → #47   P5 · 직렬 · ui/ 점유
[#48] ──→ [#27] ──→ [#31]                          보류 · ui/ 단독 점유 · P5와 동시 금지
[#81] ──→ [#49] ──→ P6 나머지                       개별 승인
```
...
| 묶음 | 이유 |
| --- | --- |
| `#106`(진행 중) · `#67` | 둘 다 계가기(`BoardAreaScorer`·`FinalScoreResult`)를 고친다 → `#67`은 `#106`이 main에 들어간 뒤 |
...
| P5 전부(`#40`~`#47`) · `#48` · `#27` | 전부 `ui/`·`GoCoachApp.kt`를 만진다. P5 안에서도 직렬 |
...
106. **접바둑 보정 규칙의 단일 출처** (AI 모델: Sonnet, 노력정도: 낮음) — 사용자 질문(2026-09-26)에서 나옴, **동작 변경 없음** · 🔵 **조율 스레드가 진행 중(2026-09-26) — 다른 세션은 집지 않는다**
...
> 🟢 표시가 붙은 것(`#43`·`#46`·`#31`·`#81`, 보류 중인 `#48`·`#27`)만 실기가 필요 없다.
...
74. 🔴 **5계층이 취소를 다시 삼킨다 — `#16`이 절반만 닫혔다** (AI 모델: Opus, 노력정도: 중간)
    · **선행** 사용자 결정(「사용자 결정 대기」의 시간 초과 동작) · **동시 금지** `#17`
    ...
    · 🧪 **실기**(폰 권장 — 실제 KataGo): ① 평소 대국 20수 — AI가 매번 둔다 ② AI가 생각하는 중에 무르기 → 곧바로 멈추고 판이
      ...
      ④ 가장 느린 조건(최고 단계·긴 생각 시간)에서 한 수 대기 시간이 설정 시간을 크게 넘지 않고, 시간 초과 때는 정한 동작대로 된다.
...
> 권장 순서 **43 → 40 → 41 → 42 → 44 → 45 → 46 → 47**. 번호 순이 아니라 이 순서다(`#43`은 `#42`보다 먼저, `#46`은 `#45` 뒤).
...
46. **예산 지표 교체** (AI 모델: Opus, 노력정도: 중간) — 45 뒤
    · **선행** `#45` · **동시 금지** P5 나머지 · 🟢 **실기 불필요**(가드·설정 파일)
...
### E. 보류 — 착수 시점은 사용자 결정 (`ui/` 단독 점유)

> 설계는 끝났다. 착수는 「사용자 결정 대기」에서 정한다. 착수 때는 설계의 S0 재실측부터.

48. **`UiStrings` 분해 — 차단 해제** (AI 모델: Opus, 노력정도: 최대) — **27보다 먼저**
    · ⏸️ **사용자 결정 대기**(착수 시점) · **동시 금지** P5 전부·`#27` · 🟢 **실기 불필요**
...
27. **`ui` 패키지 122파일 분할** (AI 모델: Opus, 노력정도: 최대) — **48 뒤에**
    · ⏸️ **사용자 결정 대기**(착수 시점) · **선행** `#48` · **동시 금지** P5 전부 · 🟢 **실기 불필요**
...
| 항목 | 정해 주실 것 | 정하기 전까지 |
| --- | --- | --- |
| `#48`·`#27`(→`#31`) | **`ui/` 단독 점유 창을 언제 열까** — `#48` 8~10시간, `#27` 5.5~8시간. 그동안 `ui/`를 만지는 다른 일(P5 포함)은 멈춘다. 설계는 끝났고 실기는 필요 없다(골든·컴파일이 증명) | 보류(2026-09-26 결정). 신규 문구는 위성 Map 우회로로 계속 넣는다 — 실제 여유가 필드 4개뿐이다 |
| `#74` | **AI 착수 탐색이 시간 초과되면 어떻게 할까** — ⓐ 지금처럼 genMove로 한 번 더(대기 최대 두 배) ⓑ 짧은 예산으로 genMove 한 번 ⓒ "수를 찾지 못했다" 알림 + 다시 시도. 사용자가 **취소**(무르기·나가기)한 경우는 즉시 멈추는 것으로 정해져 있고 결정 대상이 아니다 | 착수 세션이 수정 전 빌드에서 실제 대기 시간을 재어 추천안과 함께 묻는다 |
| `#104` 후속(선택) | 무르기 직후 버튼 넷(형세보기·추천 수·기권·통과)이 잠깐 꺼졌다 켜지는 깜빡임을 없앨까 — 짧은 busy에는 버튼을 끄지 않는 식. 사용자 관찰(2026-09-26): *"플레이에 큰 영향 없음"* | 하지 않는다 |
