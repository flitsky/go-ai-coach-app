...
71. **androidTest가 공용 픽스처를 볼 수 없다** (AI 모델: Opus, 노력정도: 중간) — 🔒 **다른 세션이 맡았다**(2026-09-24)
    ...
    · ⚠️ **`#20`의 실질 선행조건이다.** `#20`의 ⚠️가 *"픽스처(#10)가 먼저다"* 라고만 적은 것은
      **androidTest를 몰랐기 때문**이고, `#10`은 `commonTest`만 닫았다. 즉 그 전제는 **절반만 충족**이었다.
      → 이 순서를 못 지킨 채 `#20`을 먼저 돌렸으므로, `#20`이 손 페이크 2벌의 override를 맞추고
      이 일감이 **그 둘을 통째로 지운다.** 헛일 한 번이 순서를 뒤집은 값이다.
    · ⚠️ 방식 판단(Gradle 모듈 신설 vs 소스셋 `srcDir` 공유)은 **`#20`이 만드는 공통 계약 테스트 스위트가
      어디 놓이는지**와 맞물린다. 그걸 보고 정한다.
