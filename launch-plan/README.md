...
**빌드 완료(2026-08-30 갱신)**: 8/29 빌드(803)는 **올리기 전에 낡았다** — 그날 저녁까지 홈과
대국 화면이 크게 바뀌었다(백로그 #34~#38). 그래서 8/30에 다시 빌드했고, **업로드 대상은 아래
804다.** 803은 콘솔에 올라간 적이 없다.
...
| versionCode / versionName | **804 / 0.8.4** (`bump-version`이 0.8.3 → 0.8.4로 자동 증가) |
| targetSdk / minSdk | **36 / 26** |
| 빌드 명령 | `make play-internal-aab TARGET=emu` — ⚠️ 기기가 둘 연결돼 있으면 `doctor`가 막는다 |

수치는 추정이 아니라 **번들 안의 매니페스트를 직접 열어** 확인한 것이다(`versionName=0.8.4`) —
