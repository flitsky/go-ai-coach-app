...
**빌드 완료(2026-08-30 갱신 3)**: 같은 날 빌드가 네 번 갈렸다 — 803은 저녁까지 홈·대국 화면이
바뀌어(백로그 #34~#38) 낡았고, 804는 1회권 결함 둘(#43·#44)과 홈 카드 순서·캐릭터 문구
(#45·#46)로, 805는 캐릭터 이름 다국어화(#32)로 대체됐다. **업로드 대상은 아래 806 하나이며,
803·804·805는 콘솔에 올라간 적이 없다.**

⚠️ **AAB는 코드가 멎은 뒤에 만든다.** 805는 #32보다 28분 먼저 빌드돼 그 수정이 빠져 있었다 —
번들의 dex를 열어 새 문자열(`Tengen the Master`)이 없는 것을 확인하고 다시 빌드했다.
**"업로드 대상"이라고 적힌 산출물이 main보다 뒤처져 있으면 그 표기 자체가 사고를 부른다.**
스크린샷은 이번에 손대지 않았다 — #32는 한국어 화면을 바꾸지 않고 세트는 한국어다.

| 항목 | 값 |
| --- | --- |
| 산출물 | `dist/go-ai-coach-play-internal.aab` (111MB, gitignored) |
| sha256 | `2d435076a256e91013e328f1e4d9bcacbd3562178ea87e9c41e06b6f956f095f` |
| versionCode / versionName | **806 / 0.8.6** (`bump-version`이 매 빌드마다 자동 증가) |
| targetSdk / minSdk | **36 / 26** |
| 빌드 명령 | `make play-internal-aab TARGET=emu` — ⚠️ 기기가 둘 연결돼 있으면 `doctor`가 막는다 |

수치는 추정이 아니라 **번들 안의 매니페스트를 직접 열어** 확인한 것이다(`versionName=0.8.6`) —
...
다섯 장은 `v260830.1754`(805) 기준이며 806에서도 화면이 같아 그대로 쓴다.
