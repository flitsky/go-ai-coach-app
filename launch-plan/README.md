...
**빌드 완료(2026-08-30 갱신 2)**: 같은 날 빌드가 세 번 갈렸다 — 803은 저녁까지 홈·대국 화면이
바뀌어(백로그 #34~#38) 낡았고, 804는 형세 보기 1회권 결함 둘(#43·#44)과 홈 카드 순서·캐릭터
문구(#45·#46)를 넣으면서 대체됐다. **업로드 대상은 아래 805 하나다.** 803·804는 콘솔에
올라간 적이 없다.

⚠️ 이 반복이 규칙을 하나 말해 준다 — **스크린샷과 AAB는 화면 변경이 멎은 뒤에 만든다.**
#43·#44는 컷을 바꾸지 않아 재캡처가 필요 없었지만 #45·#46은 홈과 픽커를 바꿔 5장을 다시 찍었다.

| 항목 | 값 |
| --- | --- |
| 산출물 | `dist/go-ai-coach-play-internal.aab` (111MB, gitignored) |
| sha256 | `1ccbc62e5cc75b506e490bc841dd7829b55b56c4850e0733dea530175634a431` |
| versionCode / versionName | **805 / 0.8.5** (`bump-version`이 매 빌드마다 자동 증가) |
| targetSdk / minSdk | **36 / 26** |
| 빌드 명령 | `make play-internal-aab TARGET=emu` — ⚠️ 기기가 둘 연결돼 있으면 `doctor`가 막는다 |

수치는 추정이 아니라 **번들 안의 매니페스트를 직접 열어** 확인한 것이다(`versionName=0.8.5`) —
...
다섯 장 전부 이 빌드(`v260830.1754`) 기준으로 재캡처했다.
